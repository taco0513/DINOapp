...
test.describe('App Functionality Tests', () => {
  ...
  test('should handle authentication flow', async ({ page }) => {
    ...
    const authElements = page.locator('text=Sign in').or(
      page.locator('text=로그인').or(
        page.locator('text=Login').or(
          page.locator('[href*="auth"]')
        )
      )
    )
    ...
    const hasUserInfo = await page.locator('text=@').or(page.locator('img[alt*="User"]')).count() > 0
