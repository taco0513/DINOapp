...
class AutoDocsGenerator {
  ...
  async startWatching() {
    ...
    watcher
      ...
      .on('error', error =>
        console.error(chalk.red(`파일 모니터링 오류: ${error}`))
      );
  ...
  async generateReadme() {
    // 스마트 변수 생성
    const smartVariables = await this.templates.generateSmartVariables(
      this.projectRoot
    );
    ...
    // 템플릿을 사용하여 README 생성
    const readmeContent = this.templates.renderTemplate(
      'readme',
      smartVariables
    );
  ...
  async generateProjectStructure() {
    ...
    try {
      const structure = execSync(
        'find . -type f -name "*.js" -o -name "*.ts" -o -name "*.md" | head -20',
...
`;
  ...
  async checkMissingDocs() {
    ...
    if (missingDocs.length > 0) {
      console.log(
        chalk.yellow(`⚠️  누락된 문서 발견: ${missingDocs.join(', ')}`)
      );
...
`;
  ...
  async performDocumentReview() {
    try {
      ...
      if (reviewResults && reviewResults.overallScore < 80) {
        console.log(
          chalk.yellow(
            `⚠️ 문서 품질 개선이 필요합니다 (점수: ${reviewResults.overallScore}/100)`
          )
        );
        ...
        console.log(
          chalk.green(
            `✅ 문서 품질이 양호합니다 (점수: ${reviewResults.overallScore}/100)`
          )
        );
      }
    } catch (error) {
  ...
  async performAIReview(basicReviewResults) {
    try {
      ...
      const lowQualityFiles = basicReviewResults.files.filter(
        file => file.score < 70
      );

      for (const file of lowQualityFiles.slice(0, 3)) {
        // 최대 3개 파일만
        const filePath = path.join(this.projectRoot, file.path);
        if (fs.existsSync(filePath)) {
          const content = await fs.readFile(filePath, 'utf8');
          const aiAnalysis = await this.aiReviewer.performAIReview(
            file.path,
            content
          );
          ...
          if (aiAnalysis && aiAnalysis.recommendations.length > 0) {
            console.log(
              chalk.cyan(
                `💡 ${file.path}: ${aiAnalysis.recommendations.length}개 AI 추천사항 생성`
              )
            );
          }
        }
    ...
    reviewResults.files.forEach(file => {
      if (file.score < 80) {
        recommendations.push(
          `${file.path}: 품질 개선 필요 (${file.score}/100점)`
        );
      }
    });
