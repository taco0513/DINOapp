...
class ProblemPatternRecognizer {
  ...
  private detectDeadEndPattern(): DeadEndAnalysis {
    ...
    const matchCount = recentErrors.reduce((count, error) => {
      return (
        count + deadEndIndicators.filter(pattern => pattern.test(error)).length
      );
    }, 0);
  ...
  private analyzeTokenEfficiency(): TokenEfficiencyAnalysis {
    const averageTokensPerAttempt =
      this.tokenUsage / Math.max(this.attemptHistory.length, 1);
    ...
    return {
      ...
      efficiencyGain:
        (projectedContinuedCost - webSearchCost) / projectedContinuedCost,
      recommendation:
        projectedContinuedCost > webSearchCost * 2
          ? '웹 검색이 더 효율적입니다'
          : '현재 방식 계속 진행',
    };
  }
...
class ContextAwareTriggerSystem {
  ...
  evaluateTriggerConditions(
    ...
    // 일반적인 트리거 조건 확인
    const generalTrigger = this.checkGeneralTriggers(
      problemDescription,
      errorMessage
    );
  ...
  private generateSearchQuery(
    ...
    const patterns =
      this.technologyTriggers[technology.category][technology.name]
        ?.searchPatterns;
...
class RealTimeProblemSolvingMonitor {
  ...
  private analyzeSearchResults(searchResult: SearchResult): SolutionAnalysis {
    return {
      ...
      alternativeSolutions: this.selectAlternativeSolutions(
