...
describe('SchengenCalculator Component', () => {
  ...
  describe('Loading State', () => {
    it('should show loading skeleton while fetching data', () => {
      mockApiClient.getSchengenStatus.mockImplementation(() => new Promise(() => {}))
    ...
    it('should show loading placeholder elements', () => {
      ...
      expect(loadingElements.some(el => el.classList.contains('bg-gray-200'))).toBe(true)
    })
  })
  ...
  describe('Data Loading', () => {
    ...
    it('should handle API error gracefully', async () => {
      mockApiClient.getSchengenStatus.mockRejectedValueOnce(new Error('API Error'))
      
      render(<SchengenCalculator />)
      
      await waitFor(() => {
        expect(screen.getByText('셰겐 데이터를 불러올 수 없습니다.')).toBeInTheDocument()
      })
    })
  ...
  describe('Current Status Display', () => {
    ...
    it('should show non-compliance status badge for violation', async () => {
      mockApiClient.getSchengenStatus.mockResolvedValueOnce(mockNonCompliantData)
  ...
  describe('Progress Bar', () => {
    it('should show green progress bar for normal usage', async () => {
      ...
      await waitFor(() => {
        const progressBar = screen.getByText('45/90일').parentElement?.querySelector('[style*="width"]')
    ...
    it('should show yellow progress bar for high usage', async () => {
      ...
      await waitFor(() => {
        const progressBar = screen.getByText('80/90일').parentElement?.querySelector('[style*="width"]')
    ...
    it('should show red progress bar for exceeded usage', async () => {
      ...
      await waitFor(() => {
        const progressBar = screen.getByText('95/90일').parentElement?.querySelector('[style*="width"]')
  ...
  describe('Date-specific Calculator', () => {
    ...
    it('should have default date as today', async () => {
      ...
      await waitFor(() => {
        const dateInput = screen.getByLabelText('확인할 날짜') as HTMLInputElement
