...
export function TravelManagementPage({ className }: TravelManagementPageProps) {
  ...
  const [statusFilter, setStatusFilter] = useState<
    'all' | 'completed' | 'ongoing' | 'planned'
  >('all');
  const [sortBy, setSortBy] = useState<'entryDate' | 'createdAt' | 'country'>(
    'entryDate'
  );
      ...
      const params = new URLSearchParams({
        ...
        includeOngoing:
          statusFilter === 'all' || statusFilter === 'ongoing'
            ...
            : 'false',
        includePlanned:
          statusFilter === 'all' || statusFilter === 'planned'
            ? 'true'
            : 'false',
        sortBy,
        sortOrder,
  ...
  const handleAddTrip = async (tripData: any) => {
    try {
      ...
      if (data.success) {
        ...
        setError('Failed to add trip: ' + (data.message || 'Unknown error'));
      }
    } catch (error) {
      setError('Network error occurred');
    }
  ...
  const handleUpdateTrip = async (tripData: any) => {
    ...
    try {
      ...
      if (data.success) {
        setTrips(prev =>
          prev.map(trip => (trip.id === editingTrip.id ? data.data : trip))
        );
        setEditingTrip(null);
      } else {
        setError('Failed to update trip: ' + (data.message || 'Unknown error'));
      }
    } catch (error) {
      setError('Network error occurred');
    }
                      ...
                      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm'>
                        <div>
                          ...
                          <p className='font-medium'>
                            {new Date(trip.entryDate).toLocaleDateString(
                              'ko-KR'
                            )}
                          </p>
                        </div>
                        ...
                        <div>
                          ...
                          <p className='font-medium'>
                            {trip.exitDate
                              ? new Date(trip.exitDate).toLocaleDateString(
                                  'ko-KR'
                                )
                              : '진행 중'}
                          </p>
            ...
            <div className='p-6'>
              ...
              <EnhancedTripForm
                initialData={{
                  ...
                  exitDate: editingTrip.exitDate
                    ? editingTrip.exitDate.split('T')[0]
                    : '',
