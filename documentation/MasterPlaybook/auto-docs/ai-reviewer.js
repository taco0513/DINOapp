...
class AIReviewer {
  ...
  async performAIReview(documentPath, content) {
    ...
    try {
      // 1. 기본 컨텐츠 분석
      analysis.contentAnalysis = await this.analyzeContent(
        content,
        documentPath
      );

      // 2. 문서 구조 분석
      analysis.structureAnalysis = await this.analyzeStructure(
        content,
        documentPath
      );
      ...
      if (this.config.superClaude?.mcpServers?.context7) {
        analysis.mcpInsights.context7 = await this.analyzeWithContext7(
          content,
          documentPath
        );
      }
      ...
      if (this.config.superClaude?.mcpServers?.sequential) {
        analysis.mcpInsights.sequential = await this.analyzeWithSequential(
          content,
          documentPath
        );
      }
      ...
      // 5. 품질 메트릭스 계산
      analysis.qualityMetrics = await this.calculateQualityMetrics(
        content,
        analysis
      );
      ...
      console.log(
        chalk.green(`✅ AI 분석 완료: ${analysis.overallScore}/100점`)
      );
  ...
  async analyzeContent(content, documentPath) {
    ...
    try {
      ...
      // 완성도 분석
      analysis.completenessScore = this.analyzeCompleteness(
        content,
        documentPath
      );
  ...
  async analyzeWithContext7(content, documentPath) {
    ...
    try {
      ...
      // 해당 타입의 모범 패턴과 비교
      insights.documentationPatterns = this.compareToStandardPatterns(
        content,
        docType
      );

      // 프레임워크별 문서화 규칙 준수 확인
      insights.frameworkCompliance = this.checkFrameworkCompliance(
        content,
        documentPath
      );
  ...
  async calculateQualityMetrics(content, analysis) {
    ...
    try {
      ...
      metrics.comprehensiveness = this.calculateComprehensiveness(
        content,
        analysis
      );
      ...
      metrics.maintainability = this.calculateMaintainability(
        content,
        analysis
      );
      ...
      console.warn(
        chalk.yellow(`품질 메트릭스 계산 중 오류: ${error.message}`)
      );
    }
    ...
    try {
      ...
      if (analysis.contentAnalysis.readabilityScore < 70) {
        recommendations.push({
          ...
          title: '읽기 쉬움 개선',
          description:
            '문장을 더 간결하게 만들고, 전문 용어에 설명을 추가하세요.',
          impact: 'high',
          effort: 'medium',
        });
      }
      ...
      Object.entries(analysis.qualityMetrics).forEach(([metric, score]) => {
        if (score < 70) {
          recommendations.push(
            this.generateMetricRecommendation(metric, score)
          );
        }
      });
  ...
  analyzeCompleteness(content, documentPath) {
    ...
    const presentSections = this.extractHeaders(content).map(h =>
      h.text.toLowerCase()
    );
  ...
  calculateAIScore(analysis) {
    ...
    if (analysis.qualityMetrics) {
      const metricsScore =
        Object.values(analysis.qualityMetrics).reduce(
          (sum, score) => sum + score,
          0
        ) / Object.keys(analysis.qualityMetrics).length;
  ...
  async saveAIAnalysis(analysis) {
    ...
    const summaryPath = path.join(
      this.aiReviewPath,
      `summary-${path.basename(analysis.path, '.md')}-${timestamp}.md`
    );
