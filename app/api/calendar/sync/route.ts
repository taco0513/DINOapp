  ...
  return withGmailAuth(request, async (session, request) => {
    try {
      ...
      if (preventDuplicates) {
        ...
        const duplicateCount = duplicateChecks.filter(
          check => check.hasDuplicates
        ).length;
      ...
      return NextResponse.json({
        ...
        skipped: preventDuplicates
          ? travelInfos.length - filteredTravelInfos.length
          : 0,
