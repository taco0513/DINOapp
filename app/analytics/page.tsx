...
export default function AnalyticsPage() {
  ...
  const [viewMode, setViewMode] = useState<
    'overview' | 'countries' | 'timeline' | 'patterns'
  >('overview');
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (response.data.timeline) {
          const convertedTrips: Trip[] = response.data.timeline.map(
    ...
    >
      ...
      <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6'>
        ...
        <div className='flex space-x-2'>
          {(['overview', 'countries', 'timeline', 'patterns'] as const).map(
            mode => (
              <Button
                key={mode}
            ...
            <>
              ...
              <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5'>
                ...
                <div className='bg-card rounded-lg border border-border p-6'>
                  ...
                  <div className='text-2xl font-bold text-foreground'>
                    {statsData.overview.totalDays}일
                  </div>
                </div>
                ...
                <div className='bg-card rounded-lg border border-border p-6'>
                  ...
                  <div className='text-2xl font-bold text-primary'>
                    {statsData.overview.schengenDays}/90일
                  </div>
                </div>
                ...
                <div className='bg-card rounded-lg border border-border p-6'>
                  ...
                  <div className='text-2xl font-bold text-foreground'>
                    {statsData.currentYear.countries}
                  </div>
                </div>
                ...
                <div className='bg-card rounded-lg border border-border p-6'>
                  ...
                  <div className='text-2xl font-bold text-foreground'>
                    {statsData.recentActivity.visits}회
                  </div>
                </div>
              </div>

              {/* Most Visited Countries */}
              {statsData.mostVisitedCountries &&
                statsData.mostVisitedCountries.length > 0 && (
                  <div className='bg-card rounded-lg border border-border p-6'>
                    <h3 className='text-lg font-semibold text-foreground mb-5'>
                      가장 많이 방문한 국가
                    </h3>
                    <div className='flex flex-col gap-3'>
                      {statsData.mostVisitedCountries.map(
              ...
              {statsData.visaTypeDistribution &&
                statsData.visaTypeDistribution.length > 0 && (
                  <div className='bg-card rounded-lg border border-border p-6'>
                    <h3 className='text-lg font-semibold text-foreground mb-5'>
                      비자 유형별 분포
                    </h3>
                    <div className='flex flex-col gap-3'>
                      {statsData.visaTypeDistribution.map(
                        (item: any, index: number) => (
                          <div
                            key={index}
                          ...
                          <div className='flex items-center gap-4'>
                            ...
                            <div>
                              ...
                              <p className='text-sm text-muted-foreground'>
                                ...
                                {exitDate
                                  ? ` - ${exitDate.toLocaleDateString('ko-KR')}`
                                  : ' (현재 체류 중)'}
                              </p>
                            </div>
                          </div>
              ...
              <div className='flex flex-col gap-3'>
                {calculateMonthlyStats().map(month => (
                  <div
                    ...
                    className='flex justify-between items-center p-3 hover:bg-muted/50 rounded-lg'
                  >
                    ...
                    {['일', '월', '화', '수', '목', '금', '토'].map(
                      (day, index) => {
                        ...
                        return (
                          ...
                          >
                            <span className='text-foreground w-12'>
                              {day}요일
                            </span>
                        ...
                        <>
                          <div className='text-center'>
                            ...
                            <p className='text-xl font-bold text-foreground'>
                              {patterns.seasonCounts.spring}회
                            </p>
                          </div>
                          <div className='text-center'>
                            ...
                            <p className='text-xl font-bold text-foreground'>
                              {patterns.seasonCounts.summer}회
                            </p>
                          </div>
                          <div className='text-center'>
                            <div className='text-3xl mb-2'>🍂</div>
                          ...
                          <div className='text-center'>
                            ...
                            <p className='text-xl font-bold text-foreground'>
                              {patterns.seasonCounts.winter}회
                            </p>
                          </div>
                        </>
                      );
  ...
  return (
    <main className='min-h-screen bg-background'>
      {isMobile ? (
        <PullToRefresh onRefresh={loadStats}>{pageContent}</PullToRefresh>
      ) : (
        pageContent
      )}
    </main>
