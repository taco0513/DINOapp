...
const koreanAirPatterns: EmailPattern[] = [
  {
    ...
    bodyPatterns: {
      flightNumber: [/KE\s*(\d+)/i, /항공편\s*(?:번호)?:?\s*KE\s*(\d+)/i],
      departureDate: [
...
const asianaPatterns: EmailPattern[] = [
  {
    ...
    bodyPatterns: {
      flightNumber: [/OZ\s*(\d+)/i, /항공편:?\s*OZ\s*(\d+)/i],
      departureDate: [
...
const jalPatterns: EmailPattern[] = [
  {
    ...
    bodyPatterns: {
      flightNumber: [/JL\s*(\d+)/i, /Flight:?\s*JL\s*(\d+)/i],
      departureDate: [
...
const bookingPatterns: EmailPattern[] = [
  {
    ...
    bodyPatterns: {
      ...
      hotelName: [/호텔:?\s*([가-힣\s\w]+)/i, /Property:?\s*([A-Za-z\s]+)/i],
      confirmation: [/예약\s*번호:?\s*(\d+)/i, /Booking\s*number:?\s*(\d+)/i],
    },
...
export const COMMON_DATE_PATTERNS = {
  ...
  japanese: [/(\d{4})年(\d{1,2})月(\d{1,2})日/g],
};
