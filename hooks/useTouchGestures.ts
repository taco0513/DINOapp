...
export function useTouchGestures(options: TouchGestureOptions = {}) {
  ...
  const handleTouchMove = useCallback((e: TouchEvent) => {
    ...
    // Pull to refresh 감지
    if (onPullToRefresh && deltaY > 0 && !gestureStateRef.current.isDragging) {
      const scrollTop = window.pageYOffset || document.documentElement.scrollTop
      
      if (scrollTop === 0) {
        gestureStateRef.current.isPulling = true
        gestureStateRef.current.pullDistance = Math.min(deltaY, pullThreshold * 1.5)
        
        // 임계점 도달 시 햅틱 피드백
        if (deltaY >= pullThreshold && gestureStateRef.current.pullDistance < pullThreshold) {
          triggerHapticFeedback('medium')
        }
  ...
  const handleTouchEnd = useCallback((e: TouchEvent) => {
    ...
    // Pull to refresh 처리
    if (gestureStateRef.current.isPulling && gestureStateRef.current.pullDistance >= pullThreshold) {
