...
describe('CSRF Protection', () => {
  ...
  describe('CSRFMiddleware', () => {
    ...
    it('should handle custom configuration', async () => {
      ...
      // Test skip path
      const webhookRequest = new NextRequest('http://localhost:3000/api/webhook', {
        method: 'POST',
      })
  ...
  describe('SynchronizerToken', () => {
    ...
    it('should reject invalid token', async () => {
      ...
      const isValid = await synchronizer.validateToken(sessionId, 'wrong-token')
      expect(isValid).toBe(false)
    })
  ...
  describe('Integration tests', () => {
    it('should protect API endpoints end-to-end', async () => {
      ...
      const getRequest = new NextRequest('http://localhost:3000/api/csrf-token')
      ...
      const postRequest = new NextRequest('http://localhost:3000/api/protected', {
        method: 'POST',
        headers: {
          'X-CSRF-Token': token,
          'Cookie': 'sessionId=test-session',
