...
export function usePerformanceMonitor(options: PerformanceMonitorOptions = {}) {
      ...
      const measureLCP = () => {
        const observer = new PerformanceObserver((list) => {
          ...
          if (thresholds.lcp && lcp > thresholds.lcp) {
            console.warn(`LCP threshold exceeded: ${lcp}ms > ${thresholds.lcp}ms`)
          }
        })
        ...
        const observer = new PerformanceObserver((list) => {
          ...
          entries.forEach((entry: any) => {
            ...
            if (thresholds.fid && fid > thresholds.fid) {
              console.warn(`FID threshold exceeded: ${fid}ms > ${thresholds.fid}ms`)
            }
          })
        })
          ...
          entries.forEach((entry: any) => {
            if (!entry.hadRecentInput) {
              ...
              if (thresholds.cls && clsValue > thresholds.cls) {
                console.warn(`CLS threshold exceeded: ${clsValue} > ${thresholds.cls}`)
              }
            }
          })
        })
  ...
  const calculatePerformanceScore = (): number => {
    ...
    const lcpScore = largestContentfulPaint <= 2500 ? 100 : 
                   largestContentfulPaint <= 4000 ? 50 : 0
    
    const fidScore = firstInputDelay <= 100 ? 100 :
                    firstInputDelay <= 300 ? 50 : 0
    
    const clsScore = cumulativeLayoutShift <= 0.1 ? 100 :
