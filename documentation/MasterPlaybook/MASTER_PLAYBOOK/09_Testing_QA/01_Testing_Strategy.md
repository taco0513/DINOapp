...
export class UserFactory {
  static create(options: UserFactoryOptions = {}): Partial<User> {
    return {
      ...
      metadata: {
        ...
        registrationSource: faker.helpers.arrayElement([
          'web',
          'mobile',
          'api',
        ]),
  ...
  static async createManyInDb(
    count: number,
    options: UserFactoryOptions = {}
  ): Promise<User[]> {
    const promises = Array.from({ length: count }, () =>
      this.createInDb(options)
    );
...
export class ProductFactory {
  static create(options: any = {}): any {
    return {
      ...
      stockQuantity:
        options.stockQuantity || faker.number.int({ min: 0, max: 100 }),
      images:
        options.images || Array.from({ length: 3 }, () => faker.image.url()),
      tags:
        options.tags ||
...
export class CoverageAnalyzer {
        ...
        Object.entries(metrics).forEach(([type, pct]) => {
          if (
            ...
            this.coverageThresholds[
              type as keyof typeof this.coverageThresholds
            ]
          ) {
            console.log(
  ...
  private printCoverageMetrics(label: string, data: CoverageData): void {
    ...
    console.log(
      `  라인(Lines): ${data.lines.pct}% (${data.lines.covered}/${data.lines.total})`
    );
  }
  ...
  private findLowCoverageFiles(
    ...
    Object.entries(files).forEach(([file, data]) => {
      ...
      const hasLowCoverage = Object.entries(metrics).some(
        ([type, pct]) =>
          pct <
          this.coverageThresholds[type as keyof typeof this.coverageThresholds]
      );
  ...
  private async trackCoverageTrend(
