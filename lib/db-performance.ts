...
class DatabasePerformanceMonitor {
  ...
  getAverageQueryTime(): number {
    ...
    const total = this.queryMetrics.reduce((sum, metric) => sum + metric.duration, 0)
  ...
  getPerformanceReport() {
    ...
    return {
      ...
      slowQueriesPercentage: totalQueries > 0 ? (slowQueries.length / totalQueries) * 100 : 0,
      averageQueryTime: averageTime,
      slowestQuery: this.queryMetrics.length > 0 
        ? this.queryMetrics.reduce((max, metric) => metric.duration > max.duration ? metric : max)
        : null
    }
  }
}
...
export class OptimizedQueries {
  ...
  static async getUserWithTrips(prisma: PrismaClient, userId: string, limit: number = 50) {
    ...
    DatabasePerformanceMonitor.getInstance().trackQuery('getUserWithTrips', duration, { userId, limit })
  ...
  static async getSchengenVisitsOptimized(
    ...
    const cacheKey = this.getCacheKey('getSchengenVisits', { userId, fromDate, toDate })
    const cached = this.getFromCache(cacheKey)
    if (cached) return cached
    ...
    const duration = Date.now() - startTime
    DatabasePerformanceMonitor.getInstance().trackQuery('getSchengenVisitsOptimized', duration, { userId, fromDate, toDate })
  ...
  static async getCountryStatistics(prisma: PrismaClient, userId: string) {
    ...
    const duration = Date.now() - startTime
    DatabasePerformanceMonitor.getInstance().trackQuery('getCountryStatistics', duration, { userId })
  ...
  static async createMultipleVisits(prisma: PrismaClient, visits: any[]) {
    ...
    const duration = Date.now() - startTime
    DatabasePerformanceMonitor.getInstance().trackQuery('createMultipleVisits', duration, { count: visits.length })
...
export class DatabaseMaintenance {
  ...
  static async analyzeQueryPerformance(prisma: PrismaClient) {
    try {
      ...
      return {
        slowQueries,
        recommendations: this.generatePerformanceRecommendations(slowQueries as any[])
      }
    } catch (error) {
  ...
  private static generatePerformanceRecommendations(slowQueries: any[]): string[] {
    ...
    slowQueries.forEach(query => {
      if (query.mean_time > 1000) {
        recommendations.push(`Critical: Query taking ${query.mean_time}ms on average needs optimization`)
      } else if (query.mean_time > 500) {
        recommendations.push(`Warning: Query taking ${query.mean_time}ms could be optimized`)
      }
    })
