...
class RiskDashboard {
  ...
  private calculateHealthScore(risks: Risk[]): number {
    ...
    const penalty = risks.reduce(
      (total, risk) => total + weights[risk.severity],
      0
    );
...
class RiskKnowledgeBase {
  ...
  async learnFromHistory(historicalRisks: HistoricalRisk[]): Promise<void> {
...
_Risk Identification: 위험을 미리 보는 자가 프로젝트를 성공으로 이끈다_
