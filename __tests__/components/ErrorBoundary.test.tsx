...
jest.mock('lucide-react', () => ({
  ...
  Home: ({ className }: { className?: string }) => (
    <div data-testid="home" className={className}>Home</div>
  ),
}))
...
jest.mock('@/components/ui/alert', () => ({
  ...
  AlertTitle: ({ children }: any) => <div className="alert-title">{children}</div>,
}))
...
describe('ErrorBoundary Component', () => {
  ...
  describe('Persistent Errors', () => {
    it('should show persistent issue warning after multiple errors', () => {
      const ErrorBoundaryWrapper = () => {
        ...
        return (
          <div>
            <button onClick={() => setErrorCount(c => c + 1)}>Trigger Error</button>
  ...
  describe('UI Components', () => {
    ...
    it('should apply correct button variants', () => {
      ...
      expect(screen.getByRole('button', { name: /try again/i })).toHaveAttribute('data-variant', 'default')
      expect(screen.getByRole('button', { name: /reload page/i })).toHaveAttribute('data-variant', 'outline')
      expect(screen.getByRole('button', { name: /go home/i })).toHaveAttribute('data-variant', 'ghost')
    })
  })
})
...
describe('useErrorHandler Hook', () => {
  ...
  it('should handle errors with errorInfo', () => {
    const TestComponent = () => {
      ...
      const triggerError = () => {
        handleError(new Error('Hook test error'), { componentStack: 'test stack' } as any)
      }
