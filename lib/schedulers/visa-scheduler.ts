...
export class VisaExpiryScheduler {
  ...
  async runDailyCheck(): Promise<void> {
    if (this.isRunning) {
      console.log('Visa expiry check is already running, skipping...');
      return;
    }
    ...
    try {
      console.log(`[${new Date().toISOString()}] Starting daily visa expiry check...`);

      // 만료 예정 비자 확인 및 알림 발송
      await visaAlerts.checkExpiringVisas();

      const duration = Date.now() - startTime;
      console.log(`[${new Date().toISOString()}] Daily visa expiry check completed in ${duration}ms`);
  ...
  async runTimeBasedCheck(timeSlot: 'morning' | 'evening'): Promise<void> {
    ...
    try {
      console.log(`[${new Date().toISOString()}] Starting ${timeSlot} visa expiry check...`);
      ...
      const duration = Date.now() - startTime;
      console.log(`[${new Date().toISOString()}] ${timeSlot} visa expiry check completed in ${duration}ms`);
  ...
  async runWeeklySummary(): Promise<void> {
    ...
    try {
      console.log(`[${new Date().toISOString()}] Starting weekly visa summary...`);
      ...
      const duration = Date.now() - startTime;
      console.log(`[${new Date().toISOString()}] Weekly visa summary completed in ${duration}ms`);
