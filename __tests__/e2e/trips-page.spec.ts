...
test.describe('Trips Page E2E Tests', () => {
  ...
  test('should show trip cards with details', async ({ page }) => {
    ...
    const firstTripCard = page.locator('div').filter({ hasText: 'France' }).first()
  ...
  test('should validate date range', async ({ page }) => {
    ...
    await expect(page.locator('text=출국일은 입국일 이후여야 합니다')).toBeVisible()
  })
  ...
  test('should handle empty trips state', async ({ page }) => {
    ...
    await expect(page.locator('text=첫 번째 여행을 추가해보세요')).toBeVisible()
  })
  ...
  test('should handle API errors', async ({ page }) => {
    ...
    await expect(page.locator('text=여행 기록을 불러오는 중 오류가 발생했습니다')).toBeVisible()
  })
