 ...
 */
export function createTravelEvents(
  ...
  if (travelInfo.departureDate) {
    ...
    if (departureDate) {
      const departureLocation = travelInfo.departure
        ? airportCodes[travelInfo.departure as keyof typeof airportCodes] ||
          travelInfo.departure
        : '출발지';

      const destinationLocation = travelInfo.destination
        ? airportCodes[travelInfo.destination as keyof typeof airportCodes] ||
  ...
  try {
    for (const travelInfo of travelInfos) {
      ...
      if (travelInfo.confidence < 0.4) {
        result.errors.push(
          `낮은 신뢰도로 인해 건너뜀: ${travelInfo.subject} (${Math.round(travelInfo.confidence * 100)}%)`
        );
        continue;
      }
    ...
    result.success = false;
    result.errors.push(
      `동기화 중 오류: ${error instanceof Error ? error.message : '알 수 없는 오류'}`
    );
  }
...
export async function findExistingTravelEvents(
  ...
  try {
    ...
    return (
      response.data.items
        ?.filter(event => event.description?.includes(emailId))
        .map(event => ({
