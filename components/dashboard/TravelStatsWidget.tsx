...
export const _TravelStatsWidget = memo<TravelStatsWidgetProps>(
  ({ className = '' }) => {
    ...
    const _processTravelData = (trips: CountryVisit[]): TravelStats => {
      ...
      const countryVisits = trips.reduce(
        (acc, trip) => {
      ...
      const yearlyData = trips.reduce(
        (acc, trip) => {
          ...
          const days =
            tripDurations.find(t => t.country === trip.country)?.days || 0;
        ...
        {} as Record<
          number,
          { trips: number; countries: Set<string>; totalDays: number }
        >
      );
      ...
      const monthlyData = trips.reduce(
        (acc, trip) => {
          const month = new Date(trip.entryDate).toLocaleDateString('ko-KR', {
            month: 'short',
          });
      ...
      const continentData = trips.reduce(
        (acc, trip) => {
      ...
      const continentDistribution = Object.entries(continentData).map(
        ([continent, data]) => ({
          continent,
          countries: data.countries.size,
          visits: data.visits,
            ...
            <div className='space-y-6'>
              ...
              <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
                ...
                <div className='text-center'>
                  ...
                  <div className='text-sm text-secondary flex items-center justify-center gap-1'>
                    <Calendar className='h-3 w-3' />총 일수
                  </div>
                </div>
                <div className='text-center'>
                  <div className='text-2xl font-bold text-primary'>
                    {stats.averageTripLength}
                  </div>
                  <div className='text-sm text-secondary flex items-center justify-center gap-1'>
                ...
                <div className='px-4 pb-4'>
                  <div className='flex items-center justify-between mb-2'>
                    <span className='text-sm'>
                      사용: {stats.schengenUsage.totalDays}일
                    </span>
                    <span className='text-sm'>
                      남은: {stats.schengenUsage.remainingDays}일
                    </span>
                  </div>
                    ...
                    <div
                      className={`h-2 rounded-full ${
                        stats.schengenUsage.compliance
                          ? stats.schengenUsage.totalDays > 72
                            ? 'bg-yellow-500'
                            : 'bg-green-500'
                          : 'bg-red-500'
                      }`}
                      style={{
                        width: `${Math.min((stats.schengenUsage.totalDays / 90) * 100, 100)}%`,
                      }}
                    />
                  </div>
                  <p
