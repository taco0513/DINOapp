...
class PerformanceTestSuite {
  ...
  async testAPIEndpoints() {
    ...
    const endpoints = [
      ...
      { path: '/api/schengen/calculate', method: 'POST', name: 'Schengen Calculator' },
  ...
  async benchmarkEndpoint(endpoint) {
    ...
    if (times.length > 0) {
      ...
      console.log(`    ✅ ${endpoint.name}: ${stats.avg.toFixed(2)}ms avg, ${stats.p95.toFixed(2)}ms p95`);
    }
  }
  ...
  async testCacheMemory() {
    ...
    console.log(`    ✅ Cache Test: ${Math.round(iterations / ((endTime - startTime) / 1000))} ops/sec`);
  }
  ...
  async testDataProcessingMemory() {
    ...
    this.results.tests.push({
      ...
      recordsProcessed: datasets.reduce((total, dataset) => total + dataset.length, 0)
    });

    console.log(`    ✅ Data Processing: ${datasets.reduce((total, dataset) => total + dataset.length, 0)} records processed`);
  }
  ...
  async testConcurrentMemory() {
    ...
    console.log(`    ✅ Concurrent Operations: ${promises.length} parallel operations completed`);
  }
  ...
  async testDatabasePerformance() {
    ...
    const operations = [
      { name: 'Simple SELECT', complexity: 1, time: () => 10 + Math.random() * 20 },
      { name: 'JOIN Query', complexity: 3, time: () => 50 + Math.random() * 100 },
      { name: 'Complex Analytics', complexity: 5, time: () => 200 + Math.random() * 300 },
      { name: 'INSERT Operation', complexity: 2, time: () => 20 + Math.random() * 40 },
      { name: 'UPDATE Operation', complexity: 2, time: () => 25 + Math.random() * 45 }
    ];
  ...
  calculateMemoryStats(memoryUsages) {
    ...
    return {
      ...
      totalAllocated: heapUsages.reduce((sum, usage) => sum + Math.max(usage, 0), 0)
    };
  }
  ...
  printSummary() {
    ...
    if (report.summary.performance.apiResponseTimes.length > 0) {
      ...
      report.summary.performance.apiResponseTimes.forEach(api => {
        console.log(`  ${api.endpoint}: ${api.avgResponseTime.toFixed(2)}ms avg, ${api.p95ResponseTime.toFixed(2)}ms p95`);
      });
    }
    ...
    if (report.summary.performance.databasePerformance.length > 0) {
      ...
      report.summary.performance.databasePerformance.forEach(db => {
        console.log(`  ${db.operation}: ${db.avgTime.toFixed(2)}ms avg (complexity: ${db.complexity})`);
      });
    }

    console.log(`\nTotal Tests: ${report.summary.totalTests}`);
    console.log(`Environment: Node ${report.summary.environment.node} on ${report.summary.environment.platform}`);
  }
}
