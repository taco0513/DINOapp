...
export class MetricsCollector {
  ...
  getLatestMetrics(): SystemMetrics | null {
    ...
    return {
      ...
      requests:
        this.getAllAggregations().find(m => m.name === 'http.requests.total')
          ?.sum || 0,
      errors:
        this.getAllAggregations().find(m => m.name === 'http.errors.total')
          ?.sum || 0,
      responseTime:
        this.getAllAggregations().find(m => m.name === 'http.request.duration')
          ?.avg || 0,
      database: {
        connections: Math.floor(Math.random() * 10),
        queries:
          this.getAllAggregations().find(m => m.name === 'db.queries.total')
            ?.sum || 0,
        latency:
          this.getAllAggregations().find(m => m.name === 'db.query.duration')
            ?.avg || 0,
      },
    };
  }
