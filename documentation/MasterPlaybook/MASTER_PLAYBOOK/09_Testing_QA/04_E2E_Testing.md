...
e2e_test_characteristics:
  user_perspective:
    ...
    scope:
      ['User journeys', 'Business workflows', 'Cross-browser compatibility']
  ...
  maintenance_challenges:
    ...
    considerations:
      ['Flaky tests', 'Slow execution', 'Environment dependencies']
```
...
test.describe('사용자 인증 여정', () => {
  ...
  test.describe('회원가입 프로세스', () => {
    test('새로운 사용자가 회원가입을 완료할 수 있다', async ({ page }) => {
      ...
      await expect(signupPage.successMessage).toContainText(
        '이메일 인증을 확인해주세요'
      );

      // 6. 이메일 인증 링크 클릭 시뮬레이션
      const verificationToken = await testDataManager.getVerificationToken(
        userData.email
      );
    ...
    test('중복된 이메일로 회원가입 시 오류 메시지를 표시한다', async ({
      page,
    }) => {
    ...
    test('비밀번호 확인이 일치하지 않을 때 오류를 표시한다', async () => {
      ...
      await expect(signupPage.passwordMismatchError).toContainText(
        '비밀번호가 일치하지 않습니다'
      );
    });
  });
  ...
  test.describe('로그인 프로세스', () => {
    ...
    test('Remember Me 체크박스가 올바르게 작동한다', async ({
      page,
      context,
    }) => {
      const user = await testDataManager.createActiveUser();
      ...
      expect(rememberMeCookie?.expires).toBeGreaterThan(
        Date.now() / 1000 + 86400
      ); // 하루 이상
    });
  });
  ...
  test.describe('로그아웃 프로세스', () => {
    ...
    test('로그아웃 후 보호된 페이지에 접근하면 로그인 페이지로 리다이렉트된다', async ({
      page,
    }) => {
      const user = await testDataManager.createActiveUser();
  ...
  test.describe('비밀번호 재설정', () => {
    test('비밀번호 재설정 요청을 할 수 있다', async ({ page }) => {
      ...
      await expect(forgotPasswordPage.successMessage).toContainText(
        '비밀번호 재설정 링크를 이메일로 발송했습니다'
      );
    });

    test('비밀번호 재설정 링크로 비밀번호를 변경할 수 있다', async ({
      page,
    }) => {
      const user = await testDataManager.createActiveUser();
      const resetToken = await testDataManager.generatePasswordResetToken(
        user.email
      );
...
test.describe('전자상거래 구매 여정', () => {
  ...
  test('사용자가 상품을 검색하고 구매할 수 있다', async ({ page }) => {
    ...
    await expect(productDetailPage.cartNotification).toContainText(
      '장바구니에 추가되었습니다'
    );
    ...
    expect(totalPrice).toBe(
      parseFloat(productPrice.replace(/[^\d.]/g, '')) * 2
    );
    ...
    await expect(orderConfirmationPage.successMessage).toContainText(
      '주문이 완료되었습니다'
    );
    ...
    expect(orderDetails.total).toBe(
      totalPrice + orderDetails.shipping + orderDetails.tax
    );
  });
  ...
  test('재고 부족 상품은 구매할 수 없다', async ({ page }) => {
    ...
    await expect(productDetailPage.stockError).toContainText(
      '재고가 부족합니다'
    );
...
test.describe('시각적 회귀 테스트', () => {
  ...
  test('상품 카드 레이아웃이 일관되다', async ({ page }) => {
    ...
    // 그리드 레이아웃 전체
    await expect(productListPage.productGrid).toHaveScreenshot(
      'product-grid-electronics.png'
    );
  });
  ...
  test('로딩 상태가 올바르게 표시된다', async ({ page }) => {
    ...
    // 로딩 스피너 스크린샷
    await expect(productListPage.loadingSpinner).toHaveScreenshot(
      'loading-spinner.png'
    );
  });
  ...
  test('폼 유효성 검사 메시지가 올바르게 표시된다', async ({ page }) => {
    ...
    // 유효성 검사 메시지들
    await expect(loginPage.emailInput).toHaveScreenshot(
      'email-validation-error.png'
    );
    await expect(loginPage.passwordInput).toHaveScreenshot(
...
test.describe('성능 테스트', () => {
  ...
  test('큰 이미지 로딩이 페이지 성능에 영향을 주지 않는다', async ({
    page,
  }) => {
    // 이미지 로딩 시작 시간 기록
