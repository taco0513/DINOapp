  ...
  validateRelease(): ReleaseReadiness {
    const results = this.preReleaseChecks.map(category => {
      ...
      return {
        ...
        blockers: category.items.filter(
          item => !this.isCompleted(item) && this.isBlocker(item)
        ),
      };
    });

    const overallReadiness =
      results.reduce((sum, r) => sum + r.completionRate, 0) / results.length;

    return {
      isReady:
        overallReadiness >= 80 && results.every(r => r.blockers.length === 0),
