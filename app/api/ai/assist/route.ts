...
export async function POST(request: NextRequest) {
  try {
    ...
    // AI 응답 시뮬레이션 (실제로는 Claude API 또는 다른 AI 서비스 연동)
    const suggestions = await generateAISuggestions(query, context, mode);
...
async function generateAISuggestions(
  ...
  switch (mode) {
    case 'pair':
      return generatePairProgrammingSuggestions(query, context);
    case 'generate':
