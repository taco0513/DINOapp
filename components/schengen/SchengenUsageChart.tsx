...
export default function SchengenUsageChart({ visits = [] }: SchengenUsageChartProps) {
  // 셰겐 국가 목록
  ...
  const calculateUsageData = () => {
    ...
    for (let i = -90; i < 90; i++) {
      ...
      visits.forEach(visit => {
        ...
        const overlapStart = new Date(Math.max(windowStart.getTime(), entryDate.getTime()))
        const overlapEnd = new Date(Math.min(date.getTime(), exitDate.getTime()))
      ...
      data.push({
        ...
        country: visits.find(v => {
          ...
          return date >= entryDate && date <= exitDate && schengenCountries.includes(v.country)
        })?.country
      })
    }
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-secondary">
            현재 사용: <span className="font-medium" style={{
              color: currentUsage > 75 ? 'var(--color-error)' : 
                    currentUsage > 60 ? 'var(--color-warning)' : 'var(--color-success)'
            }}>
              {currentUsage}/90일
            </span>
          </p>
        ...
        <div className="text-right">
          ...
          <div className="font-medium" style={{
            color: (90 - currentUsage) > 15 ? 'var(--color-success)' : 'var(--color-warning)'
          }}>
            {90 - currentUsage}일
          </div>
        </div>
            ...
            return (
              <div
                ...
                style={{
                  flex: 1,
                  height: `${height}%`,
                  backgroundColor: isToday ? 'var(--color-primary)' :
                                 day.days > 75 ? 'var(--color-error)' :
                                 day.days > 60 ? 'var(--color-warning)' :
                                 day.days > 0 ? 'var(--color-success)' : 'var(--color-border)',
