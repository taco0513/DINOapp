  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='card'>
          ...
          <div>
            ...
            >
              ...
              <optgroup label='비셰겐 지역'>
                {countries
                  .filter(c => !isSchengenCountry(c.name))
                  .map(country => (
                    <option key={country.code} value={country.name}>
                      {country.flag} {country.name}
                    </option>
        ...
        <Button onClick={handleValidate}>여행 계획 검증하기</Button>

        {validation && (
          <div
            className={`mt-4 alert ${validation.canTravel ? 'alert-success' : 'alert-error'}`}
          >
      ...
      <div className='card'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-4'>
          ...
          <div className='flex items-end'>
            <Button onClick={handleFindSafeDates}>안전한 날짜 찾기</Button>
          </div>
        </div>
        ...
        {safeDates === null && desiredDuration && (
          <div className='mt-4 alert alert-warning'>
            <p>
              향후 1년 내에 {desiredDuration}일간 안전하게 여행할 수 있는 날짜를
              찾을 수 없습니다. 더 짧은 기간을 고려해보세요.
            </p>
          </div>
        )}
      </div>
      ...
      <div className='alert'>
        ...
        <ul className='text-sm text-secondary space-y-1 list-disc list-inside'>
          ...
          <li>
            안전한 날짜 찾기 기능으로 규정을 준수하는 여행 날짜를 자동으로 찾을
            수 있습니다
          </li>
