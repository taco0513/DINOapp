...
export default function GmailAnalyzer({
  ...
  const [selectedTab, setSelectedTab] = useState<'results' | 'details'>(
    'results'
  );
  ...
  useEffect(() => {
    ...
    let filtered = travelInfos.filter(
      info => info.confidence >= filters.confidence
    );
  ...
  const _startAnalysis = async () => {
    ...
    try {
      ...
      if (!analyzeResponse.ok) {
        ...
        throw new Error(
          errorData.message || '이메일 분석 중 오류가 발생했습니다.'
        );
      }
    ...
    if (confidence >= 0.8) {
      ...
      return (
        <span className='badge badge-warning'>
          중간 {Math.round(confidence * 100)}%
        </span>
      );
    } else {
      return (
        <span className='badge badge-error'>
          낮음 {Math.round(confidence * 100)}%
        </span>
      );
    }
  };
    ...
    <div className='space-y-6'>
                ...
                <div className='grid grid-cols-3 gap-4 text-sm'>
                  <div className='text-center'>
                    <div className='font-medium'>
                      {analysisProgress.totalEmails}
                    </div>
                    <div className='text-secondary'>총 이메일</div>
                  </div>
                  <div className='text-center'>
                    <div className='font-medium'>
                      {analysisProgress.processedEmails}
                    </div>
                    <div className='text-secondary'>처리됨</div>
                  </div>
                  <div className='text-center'>
                    <div className='font-medium'>
                      {analysisProgress.foundTravelEmails}
                    </div>
        ...
        <div className='card'>
          <div className='mb-4'>
            <div className='flex items-center justify-between mb-2'>
              ...
              <span className='badge'>{filteredTravelInfos.length}개 발곮</span>
            </div>
            <p className='text-secondary'>
              ...
              <div className='grid grid-cols-2 md:grid-cols-4 gap-4 mb-6'>
                ...
                <div className='card text-center'>
                  <div className='text-2xl font-bold text-success'>
                    {
                      filteredTravelInfos.filter(info => info.confidence >= 0.8)
                        .length
                    }
                  </div>
                ...
                <div className='card text-center'>
                  <div className='text-2xl font-bold text-warning'>
                    {
                      filteredTravelInfos.filter(info => info.flightNumber)
                        .length
                    }
                  </div>
              ...
              <div className='space-y-4'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                  <div className='card text-center'>
                    <div className='text-2xl font-bold text-success'>
                      {Math.round(
                        (filteredTravelInfos.reduce(
                          (sum, info) => sum + info.confidence,
                          0
                        ) /
                          filteredTravelInfos.length) *
                          100
                  ...
                  <div className='card text-center'>
                    <div className='text-2xl font-bold text-primary'>
                      {
                        filteredTravelInfos.filter(
                          info => info.departureDate && info.returnDate
                        ).length
                      }
                    </div>
      ...
      {!isAnalyzing && travelInfos.length === 0 && !error && (
        <div className='card p-8 text-center'>
          ...
          <p className='text-secondary'>
            위 설정을 확인하고 "분석 시작" 버튼을 클릭하여 Gmail에서 여햑 정보를
            추출하세요.
          </p>
        </div>
      )}
