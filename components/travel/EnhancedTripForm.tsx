...
export function EnhancedTripForm({
  ...
  const [validation, setValidation] = useState<FutureTripValidation | null>(
    null
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.maxDays || formData.maxDays < 1)
      newErrors.maxDays = '최대 체류일을 입력해주세요';

    if (
      formData.exitDate &&
      new Date(formData.exitDate) <= new Date(formData.entryDate)
    ) {
    ...
    <Card className={`p-6 ${className}`}>
      <form onSubmit={handleSubmit} className='space-y-6'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
          ...
          <div>
            <label className='block text-sm font-medium mb-2'>여행 상태</label>
            <select
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
          ...
          <div>
            ...
            <input
              ...
              onChange={e =>
                updateField('maxDays', parseInt(e.target.value) || 0)
              }
              className='w-full p-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent'
              required
            />
          ...
          <div>
            <label className='block text-sm font-medium mb-2'>여권 국가</label>
            <select
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
          ...
          <div>
            <label className='block text-sm font-medium mb-2'>숙박 유형</label>
            <select
        ...
        {validation && (
          <Card className='p-4 bg-gray-50'>
            ...
            <div className='space-y-2'>
              <div className='flex items-center space-x-2'>
                <Badge
                  variant={validation.canTravel ? 'default' : 'destructive'}
                >
              ...
              <div className='grid grid-cols-2 gap-4 mt-3 text-sm'>
                <div>
                  ...
                  <span className='font-medium ml-2'>
                    {validation.maxStayDays}일
                  </span>
                </div>
                <div>
                  <span className='text-gray-600'>여행 후 잔여일:</span>
                  <span className='font-medium ml-2'>
                    {validation.remainingDaysAfterTrip}일
                  </span>
                </div>
              </div>
            </div>
