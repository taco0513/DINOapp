...
export default function SchengenCalculator({ onDataUpdate }: SchengenCalculatorProps) {
  ...
  const calculateDaysOnDate = (date: string) => {
    ...
    let estimatedUsed = schengenData.status.usedDays
    if (daysDiff > 0) {
      // Future date - days might reset
      estimatedUsed = Math.max(0, estimatedUsed - Math.floor(daysDiff / 180) * 90)
    }
  ...
  const handleFutureTripCalculation = () => {
    ...
    const tripDays = Math.ceil((exitDate.getTime() - entryDate.getTime()) / (1000 * 60 * 60 * 24)) + 1
  ...
  const futureTripResult = futureTrip.enabled ? handleFutureTripCalculation() : null
  ...
  return (
    <div className="space-y-6">
      {/* Current Status */}
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">현재 셰겐 상태</h3>
        
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6">
          <div className="text-center p-3 sm:p-4 bg-gray-50 rounded-lg">
            <div className="text-xl sm:text-2xl font-bold text-gray-900">
              {schengenData.status.usedDays}
            </div>
            <div className="text-xs sm:text-sm text-gray-600">사용된 일수</div>
          </div>
          <div className="text-center p-3 sm:p-4 bg-blue-50 rounded-lg">
            <div className="text-xl sm:text-2xl font-bold text-blue-600">
              {schengenData.status.remainingDays}
            </div>
            <div className="text-xs sm:text-sm text-gray-600">남은 일수</div>
          </div>
          <div className="text-center p-3 sm:p-4 bg-purple-50 rounded-lg">
            <div className="text-xl sm:text-2xl font-bold text-purple-600">90</div>
        ...
        <div className="mb-4">
          <div className="flex justify-between text-sm text-gray-600 mb-2">
            <span>사용량</span>
            <span>{schengenData.status.usedDays}/90일</span>
          </div>
            ...
            <div
              className={`h-3 rounded-full transition-all ${
                schengenData.status.usedDays > 90 
                  ? 'bg-red-500' 
                  : schengenData.status.usedDays > 75 
                    ? 'bg-yellow-500' 
                    : 'bg-green-500'
              }`}
              style={{ 
                width: `${Math.min((schengenData.status.usedDays / 90) * 100, 100)}%` 
              }}
            />
          </div>
        </div>
        ...
        <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
          ...
          {schengenData.status.isCompliant ? '✅ 규정 준수' : '⚠️ 규정 위반'}
        </div>
      </div>

      {/* Date-specific Calculator */}
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">특정 날짜 계산기</h3>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div className="text-center sm:text-left">
              ...
              <div className="text-xs sm:text-sm text-blue-700">예상 사용 일수</div>
            </div>
            <div className="text-center sm:text-left">
              <div className="text-lg sm:text-xl font-bold text-blue-900">
                {selectedDateStatus.remainingDays}일
              </div>
              <div className="text-xs sm:text-sm text-blue-700">예상 남은 일수</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900">미래 여행 계획</h3>
          <label className="flex items-center">
            <input
              type="checkbox"
              checked={futureTrip.enabled}
              onChange={(e) => setFutureTrip(prev => ({ ...prev, enabled: e.target.checked }))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFutureTrip(prev => ({ ...prev, country: e.target.value }))}
                  className="w-full px-3 py-3 sm:py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-base sm:text-sm"
                >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFutureTrip(prev => ({ ...prev, entryDate: e.target.value }))}
                  className="w-full px-3 py-3 sm:py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-base sm:text-sm"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFutureTrip(prev => ({ ...prev, exitDate: e.target.value }))}
                  className="w-full px-3 py-3 sm:py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-base sm:text-sm"
                />
              </div>
            </div>
            ...
            {futureTripResult && (
              <div className={`border rounded-lg p-3 sm:p-4 ${
                ...
                <h4 className={`font-medium mb-3 text-sm sm:text-base ${
                  futureTripResult.wouldViolate ? 'text-red-900' : 'text-green-900'
                }`}>
                  {futureTripResult.wouldViolate ? '⚠️ 규정 위반 예상' : '✅ 규정 준수 예상'}
                </h4>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 text-sm">
                  <div className="text-center sm:text-left">
                    <div className="font-medium text-xs sm:text-sm">여행 일수</div>
                    <div className="text-base sm:text-lg font-bold">{futureTripResult.tripDays}일</div>
                  </div>
                  <div className="text-center sm:text-left">
                    <div className="font-medium text-xs sm:text-sm">입국 시 사용량</div>
                    <div className="text-base sm:text-lg font-bold">{futureTripResult.statusOnEntry.usedDays}/90일</div>
                  </div>
                  <div className="text-center sm:text-left">
                    <div className="font-medium text-xs sm:text-sm">여행 후 예상</div>
                    <div className={`text-base sm:text-lg font-bold ${futureTripResult.wouldViolate ? 'text-red-600' : ''}`}>
                      {futureTripResult.projectedUsed}/90일
                    </div>
                  </div>
                  <div className="text-center sm:text-left">
                    <div className="font-medium text-xs sm:text-sm">남은 일수</div>
                ...
                {futureTripResult.wouldViolate && (
                  <div className="mt-3 p-3 bg-red-100 rounded-lg">
                    <p className="text-red-800 text-xs sm:text-sm">
                      이 여행은 90일 한도를 {futureTripResult.projectedUsed - 90}일 초과합니다.
                      여행 계획을 조정하거나 더 나은 시기를 고려해보세요.
                    </p>
                  </div>
                )}
              </div>
