...
import { apiCache, rateLimiter, compressResponse, apiMonitor } from '@/lib/performance/api-cache'
...
export function performanceMiddleware(options: PerformanceMiddlewareOptions = {}) {
  ...
  return async (
    ...
    if (config.rateLimit?.enabled) {
      ...
      if (!rateLimiter.isAllowed(
        ...
        const response = NextResponse.json(
          { error: 'Rate limit exceeded', retryAfter: config.rateLimit.windowMs },
          { status: 429 }
        )
        
        response.headers.set('X-RateLimit-Limit', String(config.rateLimit.requests))
        response.headers.set('X-RateLimit-Remaining', '0')
        response.headers.set('Retry-After', String(Math.ceil((config.rateLimit.windowMs || 60000) / 1000)))
    ...
    try {
      ...
      // Record metrics
      recordMetrics(path, duration, response.status >= 400 ? 'error' : 'success')
...
async function processJSONResponse(
  ...
  try {
    ...
    if (config.cache?.enabled && 
        ...
        !config.cache.excludePaths?.some(excludePath => path.startsWith(excludePath))) {
    ...
    if (config.compression?.enabled && 
      ...
      if (compressionRatio < 0.9) { // Only if we achieve >10% compression
        originalResponse.headers.set('Content-Encoding', 'custom')
        originalResponse.headers.set('X-Compression-Ratio', compressionRatio.toFixed(2))
      }
    }
