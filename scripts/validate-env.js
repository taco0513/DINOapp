...
function validateEnvironment() {
  ...
  if (hasErrors) {
    ...
    console.log()
    console.log(chalk.yellow('💡 Please check your .env file or environment configuration.'))
    console.log(chalk.yellow('   Refer to .env.example for the complete list of variables.'))
    process.exit(1)
  } else {
    console.log(chalk.green.bold('✅ Environment validation passed!'))
    console.log()
    console.log(chalk.green(`Configured: ${present.length} required variables`))
    ...
    if (env === 'production') {
      ...
      if (secret && secret.length < 32) {
        console.log(chalk.yellow('⚠️  NEXTAUTH_SECRET should be at least 32 characters long'))
      } else if (secret) {
      ...
      if (dbUrl) {
        if (dbUrl.startsWith('postgresql://') || dbUrl.startsWith('postgres://')) {
          ...
          console.log(chalk.yellow('⚠️  SQLite detected - consider PostgreSQL for production'))
        } else {
      ...
      if (authUrl && !authUrl.startsWith('https://')) {
        console.log(chalk.yellow('⚠️  NEXTAUTH_URL should use HTTPS in production'))
      } else if (authUrl) {
