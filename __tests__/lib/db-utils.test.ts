...
const mockDbPerformanceMonitor = require('@/lib/db-performance').dbPerformanceMonitor
  ...
  describe('Country visit operations', () => {
    ...
    describe('getSchengenCountryVisits', () => {
      it('should use optimized Schengen query', async () => {
        ...
        const { OptimizedQueries } = require('@/lib/db-performance')
        OptimizedQueries.getSchengenVisitsOptimized.mockResolvedValue(mockVisits)

        const result = await getSchengenCountryVisits('user1', fromDate, toDate)

        expect(OptimizedQueries.getSchengenVisitsOptimized).toHaveBeenCalledWith(
          mockPrisma,
          'user1',
          fromDate,
          toDate
  ...
  describe('Notification settings', () => {
    describe('updateNotificationSettings', () => {
      it('should upsert notification settings with JSON serialization', async () => {
        ...
        mockPrisma.notificationSettings.upsert.mockResolvedValue(updatedSettings)
      ...
      it('should use default visa expiry days when creating new settings', async () => {
        ...
        const expectedCall = mockPrisma.notificationSettings.upsert.mock.calls[0][0]
  ...
  describe('Analytics', () => {
    describe('getUserTravelStats', () => {
      it('should calculate comprehensive travel statistics', async () => {
        ...
        const { OptimizedQueries } = require('@/lib/db-performance')
        OptimizedQueries.getCountryStatistics.mockResolvedValue(mockCountryStats)
