...
program
  ...
  .action(async options => {
    ...
    try {
      ...
      console.log(
        chalk.green('✅ 초기화 완료! 다음 명령어로 모니터링을 시작하세요:')
      );
  ...
  .action(async options => {
    ...
    if (options.show) {
      try {
        ...
        console.log(
          chalk.yellow(
            '⚠️  설정 파일이 없습니다. auto-docs init으로 초기화하세요.'
          )
        );
      }
    ...
    try {
      ...
      if (options.summary) {
        ...
        if (options.ai) {
          ...
          const reviewResults =
            await autoDocsGenerator.documentReviewer.reviewAllDocuments();
          if (reviewResults) {
...
program
  ...
  .action(async options => {
    ...
    console.log(
      chalk.white('⚙️  설정 파일:', fs.existsSync(configPath) ? '✅' : '❌')
    );
    console.log(
      chalk.white('📚 문서 폴더:', fs.existsSync(docsPath) ? '✅' : '❌')
    );
    console.log(
      chalk.white('📝 변경 로그:', fs.existsSync(changelogPath) ? '✅' : '❌')
    );
