...
test.describe('Authentication E2E Tests (Simplified)', () => {
  ...
  test('should display home page for unauthenticated users', async ({
    page,
  }) => {
    await page.goto('/');
  ...
  test('should redirect to signin page when clicking login', async ({
    page,
  }) => {
    await page.goto('/');
    ...
    await page.route('/api/stats', async route => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          success: true,
          data: {
            overview: { totalVisits: 0, totalCountries: 0, totalDays: 0 },
          },
        }),
      });
    });
    ...
    await page.route('/api/schengen', async route => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          success: true,
          data: {
            status: { usedDays: 0, remainingDays: 90, isCompliant: true },
          },
        }),
      });
    });
  ...
  test('should show loading state', async ({ page }) => {
    ...
    expect(
      isLoadingVisible ||
        (await page.locator('h1:has-text("DINO")').isVisible())
    ).toBeTruthy();
  });
