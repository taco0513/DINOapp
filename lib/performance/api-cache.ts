...
export class PerformanceCache<T = any> {
  ...
  set(key: string, value: T, ttl = 3600000): void { // 1 hour default
...
export function CacheResponse(ttl = 3600000) { // 1 hour default
...
export async function optimizedFetch(
  ...
  // Check rate limit
  if (rateLimit && !rateLimiter.isAllowed(rateLimit.key, rateLimit.limit, rateLimit.windowMs)) {
  ...
  if (deduplicate) {
    return await requestDeduplicator.deduplicate(cacheKey, makeRequestWithRetry)
  }
