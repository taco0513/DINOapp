    ...
    const initializeObserver = () => {
      try {
        ...
        observerRef.current.observe({ 
          entryTypes: ['largest-contentful-paint', 'first-input', 'layout-shift', 'paint', 'navigation', 'resource'] 
        })
      } catch (error) {
    ...
    const handlePerformanceEntry = (entry: PerformanceEntry) => {
      ...
      switch (entry.entryType) {
        ...
        case 'navigation':
          ...
          newMetrics.domContentLoaded = navEntry.domContentLoadedEventEnd - navEntry.fetchStart
          break
  ...
  const reportMetrics = async (metrics: PerformanceMetrics) => {
    ...
    try {
      ...
      if (typeof window !== 'undefined' && (window as any).metricsCollector) {
        ...
        if (metrics.cls) collector.histogram('web_vitals_cls', metrics.cls * 1000) // Convert to integer
        ...
        // Custom metrics
        if (metrics.pageLoadTime) collector.histogram('page_load_time', metrics.pageLoadTime)
        if (metrics.memoryUsage) collector.gauge('memory_usage_mb', metrics.memoryUsage)
      }
    ...
    <div className="fixed bottom-4 right-4 bg-black bg-opacity-90 text-white p-4 rounded-lg text-xs font-mono max-w-sm z-50">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className={`px-2 py-1 rounded text-xs ${
          score >= 90 ? 'bg-green-600' : score >= 75 ? 'bg-yellow-600' : 'bg-red-600'
        }`}>
          Score: {score}
        </div>
      </div>
      ...
      {warnings.length > 0 && (
        <div className="mt-2 pt-2 border-t border-gray-600">
          ...
          {warnings.map((warning, i) => (
            <div key={i} className="text-red-300 text-xs">• {warning}</div>
          ))}
        </div>
      )}
    </div>
