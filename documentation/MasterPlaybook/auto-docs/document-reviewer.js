  ...
  async reviewAllDocuments() {
    ...
    try {
      ...
      for (const filePath of docFiles) {
        ...
        results.summary.criticalIssues += fileReview.issues.filter(
          i => i.severity === 'critical'
        ).length;
        results.summary.warnings += fileReview.issues.filter(
          i => i.severity === 'warning'
        ).length;
        results.summary.suggestions += fileReview.issues.filter(
      ...
      console.log(
        chalk.green(
          `✅ 문서 리뷰 완료: ${docFiles.length}개 파일, ${results.summary.totalIssues}개 이슈 발견`
        )
      );
      ...
      while ((match = linkPattern.exec(line)) !== null) {
        ...
        if (
          ...
          const targetPath = path.resolve(
            path.dirname(path.join(this.projectRoot, filePath)),
            linkUrl
          );
    ...
    try {
      ...
      for (const docFile of docFiles) {
        const content = await fs.readFile(
          path.join(this.projectRoot, docFile),
          'utf8'
        );
        ...
        const versionMatches =
          content.match(/버전[:\s]*([0-9]+\.[0-9]+\.[0-9]+)/g) ||
  ...
  getQualityGrade(score) {
    if (score >= 90) {
      ...
      return {
        ...
        label: '우수 (A)',
        description:
          '문서 품질이 좋습니다. 몇 가지 소소한 개선으로 완벽해질 수 있습니다.',
      };
    } else if (score >= 70) {
      return {
        emoji: '🥈',
        label: '양호 (B)',
        description:
          '문서 품질이 양호합니다. 주요 이슈들을 해결하면 더 나아질 것입니다.',
      };
    } else if (score >= 60) {
      return {
        emoji: '🥉',
        label: '보통 (C)',
        description:
          '문서 품질이 평균적입니다. 개선이 필요한 부분들이 있습니다.',
      };
    } else {
      return {
        emoji: '⚠️',
