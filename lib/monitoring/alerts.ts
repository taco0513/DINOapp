...
export function checkPerformanceAlerts(metrics: {
  ...
  if (metrics.responseTime && metrics.responseTime > alertThresholds.performance.responseTime) {
    alerts.push({
      type: 'warning',
  ...
  if (metrics.errorRate && metrics.errorRate > alertThresholds.performance.errorRate) {
    alerts.push({
      type: 'error',
      category: 'performance',
  ...
  if (metrics.cpuUsage && metrics.cpuUsage > alertThresholds.performance.cpuUsage) {
    alerts.push({
      type: 'warning',
      category: 'performance',
  ...
  if (metrics.memoryUsage && metrics.memoryUsage > alertThresholds.performance.memoryUsage) {
    alerts.push({
      type: 'error',
      category: 'performance',
...
export function checkSecurityAlerts(events: {
  ...
  if (events.failedLoginAttempts && events.failedLoginAttempts >= alertThresholds.security.failedLoginAttempts) {
    alerts.push({
      type: 'warning',
      category: 'security',
...
export function checkUsageAlerts(usage: {
  ...
  if (usage.concurrentUsers && usage.concurrentUsers > alertThresholds.usage.concurrentUsers) {
    alerts.push({
      type: 'info',
      category: 'usage',
  ...
  if (usage.requestsPerMinute && usage.requestsPerMinute > alertThresholds.usage.requestsPerMinute) {
    alerts.push({
      type: 'warning',
      category: 'usage',
  ...
  if (usage.dataTransferGB && usage.dataTransferGB > alertThresholds.usage.dataTransferGB) {
    alerts.push({
      type: 'warning',
      category: 'usage',
