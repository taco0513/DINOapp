...
export function validateAirportCode(airportCode: string): boolean {
  ...
  return (
    ...
    /^[A-Z]{3}$/.test(normalizedCode) &&
    normalizedCode in airportCodes
  );
}
...
export function adjustConfidenceByContext(
  ...
  if (
    trustedDomains.some(domain => emailContext.senderDomain.includes(domain))
  ) {
...
export function validateDataConsistency(travelInfo: ExtractedTravelInfo): {
  ...
  if (travelInfo.departureDate && travelInfo.returnDate) {
    const depDate = new Date(
      normalizeDateString(travelInfo.departureDate) || ''
    );
  ...
  if (
    ...
    !validateFlightNumber(travelInfo.flightNumber)
  ) {
  ...
  if (
    ...
    !validateBookingReference(travelInfo.bookingReference)
  ) {
    issues.push(
      `Invalid booking reference format: ${travelInfo.bookingReference}`
    );
  }
 ...
 */
export function prioritizeTravelInfo(
  travelInfos: ExtractedTravelInfo[]
): ExtractedTravelInfo[] {
  return travelInfos
    .map(info => {
      // 데이터 일관성 검증
      ...
      if (!validation.isConsistent) {
        info.confidence = Math.max(
          0,
          info.confidence - validation.issues.length * 0.1
        );
      }
 ...
 */
export function deduplicateAndMergeTravelInfo(
  travelInfos: ExtractedTravelInfo[]
): ExtractedTravelInfo[] {
    ...
    for (let i = 0; i < merged.length; i++) {
      ...
      const isSameTrip =
        (current.flightNumber &&
          ...
          current.flightNumber === existing.flightNumber) ||
        (current.bookingReference &&
          existing.bookingReference &&
          current.bookingReference === existing.bookingReference) ||
        (current.departureDate &&
          existing.departureDate &&
          current.departure &&
          existing.departure &&
          normalizeDateString(current.departureDate) ===
            normalizeDateString(existing.departureDate) &&
 ...
 * 두 여행 정보를 병합
 */
function mergeTravelInfos(
  info1: ExtractedTravelInfo,
  info2: ExtractedTravelInfo
): ExtractedTravelInfo {
  const merged = { ...info1 };
  ...
  merged.extractedData = {
    dates: [
      ...new Set([
        ...primary.extractedData.dates,
        ...secondary.extractedData.dates,
      ]),
    ],
    airports: [
      ...new Set([
        ...primary.extractedData.airports,
        ...secondary.extractedData.airports,
      ]),
    ],
    flights: [
      ...new Set([
        ...primary.extractedData.flights,
        ...secondary.extractedData.flights,
      ]),
    ],
    bookingCodes: [
      ...new Set([
        ...primary.extractedData.bookingCodes,
        ...secondary.extractedData.bookingCodes,
      ]),
    ],
  };
  ...
  merged.confidence = Math.min(
    1.0,
    Math.max(info1.confidence, info2.confidence) + 0.05
  );
