...
test.describe('Mobile Experience Tests', () => {
  ...
  test('mobile navigation should be visible and functional', async ({
    page,
  }) => {
  ...
  test('swipeable cards should work in trips list', async ({ page }) => {
    ...
    const mobileCard = page
      .locator('[class*="SwipeableCard"], .mobile-trip-card')
      .first();
    ...
    if ((await mobileCard.count()) > 0) {
      ...
      if (cardBox) {
        ...
        const editVisible =
          (await editButton.count()) > 0 && (await editButton.isVisible());
        const deleteVisible =
          (await deleteButton.count()) > 0 && (await deleteButton.isVisible());
  ...
  test('mobile modals should work properly', async ({ page }) => {
    ...
    if (modalBox) {
      ...
      await page.mouse.down();
      await page.mouse.move(modalBox.x + modalBox.width / 2, modalBox.y + 150, {
        steps: 10,
      });
      await page.mouse.up();
    ...
    const closeButton = page
      .locator('button[aria-label="닫기"], button:has-text("취소")')
      .first();
