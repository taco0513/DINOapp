...
async function checkJSDocCoverage() {
  ...
  console.log(`  📊 JSDoc 커버리지: ${coverage}% (${documentedItems}/${totalItems})`)
  console.log(`  🎯 목표: ${CONFIG.qualityThresholds.jsdocCoverage}% - ${results.passed ? '✅' : '❌'}`)
...
function extractFunctionsWithDocs(content) {
  ...
  while ((match = functionRegex.exec(content)) !== null) {
    ...
    if (name && !name.startsWith('_')) { // private 함수 제외
...
async function checkDocumentCompleteness() {
  ...
  const completeness = results.totalDocs > 0 
    ? Math.round((results.completeDocs / results.totalDocs) * 100) 
    : 0
  
  results.completeness = completeness
  results.passed = completeness >= CONFIG.qualityThresholds.documentCompleteness
  
  console.log(`  📊 문서 완성도: ${completeness}% (${results.completeDocs}/${results.totalDocs})`)
  console.log(`  🎯 목표: ${CONFIG.qualityThresholds.documentCompleteness}% - ${results.passed ? '✅' : '❌'}`)
...
async function checkLinkValidation() {
  ...
  const validationRate = results.totalLinks > 0 
  ...
  console.log(`  📊 링크 유효성: ${validationRate}% (${results.validLinks}/${results.totalLinks})`)
  console.log(`  🎯 목표: ${CONFIG.qualityThresholds.linkValidation}% - ${results.passed ? '✅' : '❌'}`)
...
async function checkCodeExamples() {
  ...
  const exampleRate = results.totalDocs > 0 
  ...
  console.log(`  📊 코드 예제 포함률: ${exampleRate}% (${results.docsWithExamples}/${results.totalDocs})`)
  console.log(`  🎯 목표: ${CONFIG.qualityThresholds.codeExamples}% - ${results.passed ? '✅' : '❌'}`)
...
function getSourceFiles(dir) {
  ...
  for (const item of items) {
    ...
    if (stat.isDirectory() && !CONFIG.excludePatterns.some(p => item.includes(p))) {
