        ...
        <Grid.Col span={8}>
          <Card shadow='sm' padding='lg'>
            ...
            <MetricChart
              ...
              realTimeData={realTimeMetrics.filter(
                m => m.metricName === 'api_requests'
              )}
              type='line'
              height={300}
            />
...
export const UserManagement: React.FC = () => {
  ...
  const { users, loading, createUser, updateUser, deleteUser, pagination } =
    useUsers();
  ...
  async checkThresholds(organizationId: string, metrics: any[]) {
    ...
    for (const metric of metrics) {
      const threshold = thresholds.find(
        t => t.metricName === metric.metricName
      );
...
export class ReportService {
  ...
  private async collectReportData(
    ...
    const [metrics, userActivity, apiUsage, errors, revenue] =
      await Promise.all([
