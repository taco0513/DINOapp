...
export default function AdvancedAnalyticsDashboard() {
  const [userBehavior, setUserBehavior] = useState<UserBehaviorData | null>(
    null
  );
  ...
  const _fetchAnalyticsData = async () => {
    try {
      ...
      const [behavior, conversion, retention] = await Promise.all([
        ...
        fetch(`/api/analytics/conversion?range=${dateRange}`).then(r =>
          r.json()
        ),
        fetch(`/api/analytics/retention?range=${dateRange}`).then(r =>
          r.json()
        ),
      ]);
          ...
          <Card>
            ...
            <CardContent>
              <div className='text-2xl font-bold'>
                {Math.floor(userBehavior.avgSessionDuration / 60)}분{' '}
                {userBehavior.avgSessionDuration % 60}초
              </div>
            ...
            <>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className='text-3xl font-bold'>
                      ₩{retentionData.lifetimeValue.toLocaleString()}
                    </div>
                    <p className='text-sm text-muted-foreground'>
                      사용자당 평균 수익
                    </p>
                    ...
                    >
                      ...
                      <Bar
                        ...
                        shape={(props: any) => {
                          ...
                          const maxWidth = 400;
                          const percentage =
                            props.users / conversionData.funnelSteps[0].users;
