...
async function handleWebVitals(data: PerformanceMetric) {
  const { metrics, url, userAgent, timestamp: _timestamp } = data;
  ...
  if (metrics.cls) {
    metricsCollector.histogram('web_vitals_cls', metrics.cls * 1000, {
      // Convert to integer
      url: new URL(url).pathname,
      device: getDeviceType(userAgent),
    });
...
async function getAPIPerformanceMetrics(timeframe: string) {
  ...
  const metrics = {
    endpoints: [
      {
        endpoint: '/api/travel/records',
        averageTime: 180,
        requests: 1245,
        errors: 12,
      },
      {
        endpoint: '/api/schengen/calculate',
        averageTime: 95,
        requests: 856,
        errors: 3,
      },
...
function generateMetricData(metric: string, timeframe: string) {
  ...
  const now = Date.now();
  const interval =
    timeframe === '1h' ? 60000 : timeframe === '24h' ? 3600000 : 86400000;
