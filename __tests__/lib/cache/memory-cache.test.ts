...
describe('Memory Cache', () => {
  ...
  describe('withCache wrapper function', () => {
    ...
    it('should handle null and undefined returns from fetcher', async () => {
      ...
      const undefinedResult = await withCache(
        'undefined-key',
        undefinedFetcher
      );
      ...
      const cachedUndefined = await withCache(
        'undefined-key',
        undefinedFetcher
      );
  ...
  describe('CacheKeys constant', () => {
    it('should provide user-specific cache keys', () => {
      ...
      expect(CacheKeys.USER_SCHENGEN_STATUS(userId)).toBe(
        'user:user123:schengen'
      );
      expect(CacheKeys.USER_NOTIFICATIONS(userId)).toBe(
    ...
    it('should provide Gmail integration cache keys', () => {
      ...
      expect(CacheKeys.GMAIL_MESSAGES(userId, query)).toContain(
        'gmail:user123:messages:'
      );
      expect(CacheKeys.GMAIL_ANALYSIS(userId, messageId)).toBe(
        'gmail:user123:analysis:msg456'
      );
    });
