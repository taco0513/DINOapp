...
export const Form = {
  ...
  Field: ({ children }) => <div className='space-y-1'>{children}</div>,

  Actions: ({ children }) => (
    <div className='flex gap-3 justify-end pt-4'>{children}</div>
  ),
};
  ...
  return (
    <Card title='로그인' subtitle='계정에 로그인하세요'>
      <Form.Root onSubmit={handleLogin}>
        ...
        <Form.Field>
          <Input label='비밀번호' type='password' name='password' required />
        </Form.Field>
