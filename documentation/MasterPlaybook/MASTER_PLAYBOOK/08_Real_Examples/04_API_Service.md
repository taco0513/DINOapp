...
export class PostService {
  ...
  async findById(postId: string, userId?: string): Promise<PostDto | null> {
    ...
    return this.transformPostToDto(
      post,
      Array.isArray(post.likes) ? post.likes.length > 0 : false
    );
  }
  ...
  async getFeed(
    userId: string,
    options: FeedOptions
  ): Promise<{ posts: PostDto[]; hasMore: boolean }> {
...
export class NotificationService {
  ...
  async create(
    ...
    if (
      userSettings.pushEnabled &&
      this.shouldSendPushNotification(type, userSettings)
    ) {
