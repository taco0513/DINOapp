...
export async function GET(request: Request) {
  ...
  try {
    ...
    checks.checks.database = {
      ...
      latency: `${latency}ms`,
      connection: 'active',
      type: process.env.DATABASE_URL?.includes('sqlite')
        ? 'sqlite'
        : 'postgresql',
  ...
  const requestMetrics = metricsAggregations.filter(
    m => m.name === 'http.requests.total'
  );
  const errorMetrics = metricsAggregations.filter(
    m => m.name === 'http.errors.total'
  );
