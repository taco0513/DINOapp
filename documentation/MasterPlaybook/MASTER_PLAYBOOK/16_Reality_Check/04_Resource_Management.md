...
class RealisticTimeEstimator {
  estimateTask(task: Task): TimeEstimate {
    ...
    const totalMultiplier = Object.values(realityMultipliers).reduce(
      (acc, mult) => acc * mult,
      1.0
    );
...
class FundingRealityGuide {
  getFundingOptions(): FundingOptions {
    return {
      // 부트스트랩 (자력 개발)
      bootstrapping: {
        pros: ['완전한 자율성', '지분 희석 없음', '빠른 의사결정'],
        cons: ['제한된 자원', '개인 위험 부담', '성장 속도 제약'],
        timeline: '12-24개월',
      ...
      // 정부 지원
      government_support: {
        pros: ['무상 지원 또는 저금리', '지분 희석 없음', '추가 지원 연계'],
        cons: ['복잡한 절차', '엄격한 심사', '긴 대기 시간'],
      ...
      // 엔젤 투자
      angel_investment: {
        pros: ['초기 자금 확보', '경험과 네트워크', '빠른 진행'],
        cons: ['지분 희석 (10-25%)', '투자자 기대 압박', 'exit 압력'],
        timeline: '3-6개월',
        typical_amount: '1억-5억원',
        requirements: ['검증된 팀', 'MVP와 초기 트랙션', '명확한 시장 기회'],
      },
...
class TechnicalDebtManager {
  assessTechnicalDebt(): DebtAssessment {
    return {
      ...
      necessaryDebt: {
        mvp_shortcuts: {
          examples: ['하드코딩된 설정값', '간단한 에러 처리', '기본적인 UI'],
      ...
      dangerousDebt: {
        ...
        scalability_issues: {
          examples: ['N+1 쿼리 문제', '메모리 누수', '단일 장애점'],
