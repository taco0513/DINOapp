...
class DatabaseOptimizer {
  ...
  createOptimizedClient(config: ConnectionPoolConfig = {}): PrismaClient {
    ...
    url.searchParams.set(
      'pool_timeout',
      Math.floor(acquireTimeout / 1000).toString()
    );
  ...
  getPerformanceStats(): any {
    ...
    return {
      ...
      slowQueryPercentage: Math.round(
        (slowQueries.length / recentMetrics.length) * 100
      ),
  ...
  private getQueryTypeStats(metrics: QueryMetrics[]): Record<string, any> {
    ...
    for (const metric of metrics) {
      ...
      stats[type].avgTime = Math.round(
        stats[type].totalTime / stats[type].count
      );
    }
...
export class OptimizedQueries {
  ...
  async batchCreateTravelRecords(records: any[]) {
    return QueryOptimizer.batchQueries(
      records.map(
        record => () => this.prisma.travelRecord.create({ data: record })
      ),
