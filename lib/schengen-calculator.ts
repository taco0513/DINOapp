...
export function calculateSchengenStatus(
  ...
  for (const visit of relevantVisits) {
    const visitStart =
      visit.entryDate > currentPeriodStart
        ? visit.entryDate
        : currentPeriodStart;
    const visitEnd =
      visit.exitDate && visit.exitDate < today ? visit.exitDate : today;
  ...
  if (relevantVisits.length > 0) {
    ...
    if (earliestVisit) {
      nextResetDate = new Date(
        earliestVisit.entryDate.getTime() + 180 * 24 * 60 * 60 * 1000
      );
    }
  }
...
export function getSchengenWarnings(status: SchengenStatus): string[] {
  ...
  if (status.remainingDays <= 10 && status.remainingDays > 0) {
    warnings.push(
      `⚠️ 주의: 셰겐 지역 체류 가능일이 ${status.remainingDays}일 남았습니다.`
    );
  }

  if (status.remainingDays === 0 && status.isCompliant) {
    warnings.push(
      '⚠️ 셰겐 지역 체류 한도에 도달했습니다. 추가 체류는 불가능합니다.'
    );
  }
...
export function validateFutureTrip(
  ...
  if (!isSchengenCountry(plannedCountry)) {
    return {
      ...
      warnings: [],
      suggestions: [
        `${plannedCountry}는 셰겐 지역이 아니므로 90/180일 규칙이 적용되지 않습니다.`,
      ],
      maxStayDays: 90,
      violatesRule: false,
      daysUsedAfterTrip: 0,
  ...
  if (statusOnEntry.remainingDays === 0) {
    ...
    suggestions.push(
      `다음 날짜 이후 입국 가능: ${statusOnEntry.nextResetDate}`
    );
  }
  ...
  if (plannedDays > statusOnEntry.remainingDays) {
    warnings.push(
      `⚠️ 계획된 ${plannedDays}일 체류는 가능한 ${statusOnEntry.remainingDays}일을 초과합니다.`
    );
    suggestions.push(
      `최대 ${statusOnEntry.remainingDays}일까지만 체류 가능합니다.`
    );
  }
  ...
  if (!futureStatus.isCompliant) {
    ...
    if (safeDays > 0) {
      const safeExitDate = new Date(
        plannedEntry.getTime() + (safeDays - 1) * 24 * 60 * 60 * 1000
      );
      suggestions.push(
        `${plannedEntry.toLocaleDateString('ko-KR')}부터 ${safeExitDate.toLocaleDateString('ko-KR')}까지 (${safeDays}일) 체류 가능`
      );
    }
  }
...
export function getSafeTravelDates(
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    const testEndDate = new Date(
      testStartDate.getTime() + (desiredDuration - 1) * 24 * 60 * 60 * 1000
    );
