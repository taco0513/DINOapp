...
const CONFIG = {
  // 스캔할 디렉토리들
  scanDirs: ['app', 'components', 'lib', 'types'],
  // 제외할 패턴들
  excludePatterns: [
    'node_modules',
    '.next',
...
function extractExports(content) {
  const exportRegex =
    /export\s+(?:default\s+)?(?:const\s+|let\s+|var\s+|function\s+|class\s+|interface\s+|type\s+)?(\w+)/g;
  const exports = [];
  let match;
...
async function generateMarkdownDocs(documentation) {
  ...
  for (const [category, items] of Object.entries(documentation)) {
    if (items.length > 0) {
      ...
      fs.writeFileSync(path.join(CONFIG.outputDir, filename), markdown, 'utf8');
...
async function generateIndexFile(documentation) {
  ...
  // 통계 요약
  const totalFiles = Object.values(documentation).reduce(
    (sum, items) => sum + items.length,
    0
  );
  const totalFunctions = Object.values(documentation).reduce(
    (sum, items) =>
      sum + items.reduce((subSum, item) => subSum + item.functions.length, 0),
    0
  );
  const totalClasses = Object.values(documentation).reduce(
    (sum, items) =>
      sum + items.reduce((subSum, item) => subSum + item.classes.length, 0),
    0
  );
  ...
  const mainComponents = components
    .filter(c => c.functions.length > 0 || c.classes.length > 0)
    .slice(0, 10);
...
async function generateTypeDoc() {
  ...
  try {
    ...
    const typedocConfig = {
      ...
      categoryOrder: ['Components', 'Hooks', 'Utilities', 'Types', 'API', '*'],
    };
