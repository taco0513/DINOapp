...
test.describe('Trip Management E2E Tests', () => {
  ...
  test('should edit an existing trip', async ({ page }) => {
    ...
    await page.fill(
      ...
      'Updated: Extended stay in Paris'
    );
    ...
    await expect(
      page.locator('text=Updated: Extended stay in Paris')
    ).toBeVisible();
  });
  ...
  test('should validate form inputs', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Exit date must be after entry date')
    ).toBeVisible();
  });
});
...
test.describe('Schengen Calculator E2E Tests', () => {
  ...
  test('should show detailed calculation breakdown', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="calculation-details"]')
    ).toBeVisible();
  ...
  test('should provide warnings for approaching limits', async ({ page }) => {
    ...
    await page.route('/api/schengen/calculate', async route => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          warnings: [
            '주의: 셰겐 지역 체류 한도에 근접했습니다. 남은 일수: 5일',
          ],
        }),
      });
    });
    ...
    await expect(
      page.locator('[data-testid="schengen-warning"]')
    ).toBeVisible();
    await expect(
      page.locator('text=주의: 셰겐 지역 체류 한도에 근접했습니다')
    ).toBeVisible();
...
test.describe('PWA Functionality E2E Tests', () => {
  ...
  test('should show install prompt', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="install-app-button"]')
    ).toBeVisible();
  });
  ...
  test('should sync data when back online', async ({ page, context }) => {
    ...
    await expect(
      page.locator('text=Saved offline. Will sync when online.')
    ).toBeVisible();
