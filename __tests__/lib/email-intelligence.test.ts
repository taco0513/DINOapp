...
describe('Email Intelligence', () => {
  ...
  describe('adjustConfidenceByContext function', () => {
    ...
    it('should increase confidence for trusted domains', () => {
      ...
      const adjustedConfidence = adjustConfidenceByContext(baseTravelInfo, emailContext)
    ...
    it('should not exceed 1.0 or go below 0.0', () => {
      ...
      const adjustedConfidence = adjustConfidenceByContext(highConfidenceInfo, emailContext)
      ...
      const lowAdjustedConfidence = adjustConfidenceByContext(lowConfidenceInfo, negativeContext)
  ...
  describe('validateDataConsistency function', () => {
    ...
    it('should detect date inconsistencies', () => {
      ...
      expect(validation.issues).toContain('Return date must be after departure date')
    })
    ...
    it('should detect invalid flight numbers', () => {
      ...
      expect(validation.isConsistent).toBe(false)
      expect(validation.issues.some(issue => issue.includes('Invalid flight number'))).toBe(true)
    })
    ...
    it('should detect invalid airport codes', () => {
      ...
      expect(validation.isConsistent).toBe(false)
      expect(validation.issues.some(issue => issue.includes('Invalid departure airport code'))).toBe(true)
      expect(validation.issues.some(issue => issue.includes('Invalid destination airport code'))).toBe(true)
    })
    ...
    it('should detect same departure and destination', () => {
      ...
      expect(validation.isConsistent).toBe(false)
      expect(validation.issues).toContain('Departure and destination airports cannot be the same')
    })
    ...
    it('should detect dates too far in the future', () => {
      ...
      if (!validation.isConsistent) {
        expect(validation.issues.some(issue => issue.includes('2 years in the future'))).toBe(true)
      }
    })
  })
  ...
  describe('prioritizeTravelInfo function', () => {
    it('should sort by confidence', () => {
      const travelInfos: ExtractedTravelInfo[] = [
        {
          confidence: 0.5,
          extractedData: { dates: [], airports: [], flights: [], bookingCodes: [] }
        },
        {
          confidence: 0.9,
          extractedData: { dates: [], airports: [], flights: [], bookingCodes: [] }
      ...
      const travelInfos: ExtractedTravelInfo[] = [
        {
          ...
          departure: 'ICN',
          confidence: 0.7,
          extractedData: { dates: [], airports: ['ICN'], flights: ['KE123'], bookingCodes: [] }
        },
        {
          flightNumber: 'KE123',
          destination: 'NRT',
          confidence: 0.8,
          extractedData: { dates: [], airports: ['NRT'], flights: ['KE123'], bookingCodes: [] }
        }
      ]
      ...
      const travelInfos: ExtractedTravelInfo[] = [
        {
          ...
          departureDate: '2024-12-25',
          confidence: 0.6,
          extractedData: { dates: ['2024-12-25'], airports: [], flights: [], bookingCodes: ['ABC123'] }
        },
        {
          bookingReference: 'ABC123',
          returnDate: '2024-12-30',
          confidence: 0.7,
          extractedData: { dates: ['2024-12-30'], airports: [], flights: [], bookingCodes: ['ABC123'] }
        }
      ]
      ...
      const travelInfos: ExtractedTravelInfo[] = [
        {
          ...
          departure: 'ICN',
          confidence: 0.6,
          extractedData: { dates: ['2024-12-25'], airports: ['ICN'], flights: [], bookingCodes: [] }
        },
        {
          departureDate: '2024-12-25',
          departure: 'ICN',
          destination: 'NRT',
          confidence: 0.8,
          extractedData: { dates: ['2024-12-25'], airports: ['ICN', 'NRT'], flights: [], bookingCodes: [] }
        }
      ]
      ...
      const travelInfos: ExtractedTravelInfo[] = [
        {
          flightNumber: 'KE123',
          confidence: 0.8,
          extractedData: { dates: [], airports: [], flights: ['KE123'], bookingCodes: [] }
        },
        {
          flightNumber: 'OZ456',
          confidence: 0.8,
          extractedData: { dates: [], airports: [], flights: ['OZ456'], bookingCodes: [] }
        }
      ]
