...
const CountryVisitSchema = z.object({
  ...
  exitDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Invalid date format').optional().nullable(),
  visaType: z.enum([
    'Tourist',
    'Business',
    'Student',
...
const isEntryBeforeExit = (entryDate: string, exitDate: string | null): boolean => {
...
const calculateTripDuration = (entryDate: string, exitDate: string | null): number => {
  if (!exitDate) {
    // Ongoing trip - calculate days from entry to today
    const entry = new Date(entryDate)
    const today = new Date()
    return Math.ceil((today.getTime() - entry.getTime()) / (1000 * 60 * 60 * 24))
  }
...
describe('Data Validations', () => {
  ...
  describe('NotificationPreferencesSchema', () => {
    ...
    it('should reject invalid visaExpiryDays values', () => {
      const invalidPreferences = { ...validPreferences, visaExpiryDays: [0, 7, 1] }
      const result = NotificationPreferencesSchema.safeParse(invalidPreferences)
    ...
    it('should reject invalid schengenWarningThreshold', () => {
      const invalidPreferences = { ...validPreferences, schengenWarningThreshold: 0 }
      const result = NotificationPreferencesSchema.safeParse(invalidPreferences)
    ...
    it('should reject schengenWarningThreshold over 90', () => {
      const invalidPreferences = { ...validPreferences, schengenWarningThreshold: 100 }
      const result = NotificationPreferencesSchema.safeParse(invalidPreferences)
  ...
  describe('Business Logic Validations', () => {
    ...
    it('should validate rolling 180-day window', () => {
      ...
      const isOldWithinWindow = oldVisitDate >= windowStart && oldVisitDate <= today
