    ...
    try {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        if (packageJson.scripts) {
          variables.startCommand = packageJson.scripts.start
            ? 'npm start'
            : 'npm run dev';
          variables.testCommand = packageJson.scripts.test
            ? 'npm test'
            : 'npm run test';
      ...
      variables.projectStructure =
        await this.generateProjectStructure(projectRoot);
  ...
  async generateProjectStructure(projectRoot) {
    try {
      ...
      const structure = execSync(
        'find . -type f -name "*.js" -o -name "*.ts" -o -name "*.jsx" -o -name "*.tsx" -o -name "*.md" | head -20',
