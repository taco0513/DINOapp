...
describe('Google API Integration Tests', () => {
  ...
  describe('Gmail Integration', () => {
    ...
    it('should search travel emails successfully', async () => {
      ...
      const mockMessageDetails = {
        data: {
          ...
          payload: {
            ...
            body: {
              data: Buffer.from('Flight details: NYC to Paris').toString('base64')
            }
          }
        }
      }
      ...
      const gmailMock = google.gmail()
      ;(gmailMock.users.messages.list as jest.Mock).mockResolvedValue(mockMessages)
      ;(gmailMock.users.messages.get as jest.Mock).mockResolvedValue(mockMessageDetails)
  ...
  describe('Calendar Integration', () => {
      ...
      const mockCalendars = {
        data: {
          items: [
            ...
            { id: 'travel@group.calendar.google.com', summary: 'Travel Calendar' }
          ]
        }
      }

      const calendarMock = google.calendar()
      ;(calendarMock.calendarList.list as jest.Mock).mockResolvedValue(mockCalendars)
    ...
    it('should create calendar event', async () => {
      ...
      const result = await createCalendarEvent(mockAccessToken, mockEvent, 'primary')
