...
describe('TripCard Component', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(Date, 'now').mockReturnValue(new Date('2024-01-20T00:00:00Z').getTime())
  })
  ...
  describe('Rendering', () => {
    it('should render trip information correctly', () => {
      render(<TripCard trip={mockTrip} onEdit={mockOnEdit} onDelete={mockOnDelete} />)
    ...
    it('should not show Schengen badge for non-Schengen countries', () => {
      ...
      render(<TripCard trip={nonSchengenTrip} onEdit={mockOnEdit} onDelete={mockOnDelete} />)
    ...
    it('should show "현재 체류 중" for ongoing trips', () => {
      ...
      render(<TripCard trip={ongoingTrip} onEdit={mockOnEdit} onDelete={mockOnDelete} />)
  ...
  describe('Duration Calculation', () => {
    ...
    it('should calculate days correctly for ongoing trips', () => {
      const ongoingTrip = { ...mockTrip, exitDate: null }
      render(<TripCard trip={ongoingTrip} onEdit={mockOnDelete} onDelete={mockOnDelete} />)
  ...
  describe('Progress Bar Colors', () => {
    it('should show green progress bar for normal stays', () => {
      ...
      const progressBar = screen.getByText('16%').parentElement?.firstChild as HTMLElement
      expect(progressBar).toHaveStyle('background-color: var(--color-success)')
    })

    it('should show warning color when approaching limit', () => {
      const longTrip = { ...mockTrip, entryDate: '2024-01-01T00:00:00Z', exitDate: '2024-03-01T00:00:00Z' }
      render(<TripCard trip={longTrip} onEdit={mockOnEdit} onDelete={mockOnDelete} />)
      
      // 60 days = 67% of 90 days, but static mock shows warning at >80%
      // Let's test with a trip that exceeds 80%
      const warningTrip = { ...mockTrip, maxDays: 20, entryDate: '2024-01-01T00:00:00Z', exitDate: '2024-01-18T00:00:00Z' }
      const { rerender } = render(<TripCard trip={warningTrip} onEdit={mockOnEdit} onDelete={mockOnDelete} />)
      
      rerender(<TripCard trip={warningTrip} onEdit={mockOnEdit} onDelete={mockOnDelete} />)
      const progressBar = screen.getByText('85%').parentElement?.firstChild as HTMLElement
    ...
    it('should show error color and warning message when exceeding limit', () => {
      ...
      render(<TripCard trip={excessiveTrip} onEdit={mockOnEdit} onDelete={mockOnDelete} />)
      
      expect(screen.getByText('⚠️ 최대 체류 일수 초과')).toBeInTheDocument()
      const progressBar = screen.getByText('140%').parentElement?.firstChild as HTMLElement
  ...
  describe('Actions', () => {
    ...
    it('should show confirmation dialog when delete button is clicked', () => {
      ...
      expect(screen.getByText('정말 이 여행 기록을 삭제하시겠습니까?')).toBeInTheDocument()
    ...
    it('should cancel delete confirmation', () => {
      ...
      expect(screen.queryByText('정말 이 여행 기록을 삭제하시겠습니까?')).not.toBeInTheDocument()
    })
    ...
    it('should handle delete error gracefully', async () => {
      mockApiClient.deleteTrip.mockRejectedValueOnce(new Error('Delete failed'))
  ...
  describe('Edge Cases', () => {
    it('should handle trip without notes', () => {
      ...
      render(<TripCard trip={tripWithoutNotes} onEdit={mockOnEdit} onDelete={mockOnDelete} />)
    ...
    it('should handle unknown country gracefully', () => {
      ...
      render(<TripCard trip={unknownCountryTrip} onEdit={mockOnEdit} onDelete={mockOnDelete} />)
    ...
    it('should handle zero maxDays gracefully', () => {
      ...
      render(<TripCard trip={zeroMaxDaysTrip} onEdit={mockOnEdit} onDelete={mockOnDelete} />)
