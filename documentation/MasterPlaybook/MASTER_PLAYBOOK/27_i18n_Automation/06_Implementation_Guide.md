...
class DependencyInstallation {
  private readonly DEPENDENCIES = {
    ...
    ai: ['openai', '@google-cloud/translate', 'deepl-node'],
    development: ['i18next-parser', 'i18next-scanner', '@types/react-i18next'],
    quality: [
...
class HardcodingScanner {
  private patterns = {
    ...
    // HTML 속성
    htmlAttribute:
      /(title|alt|placeholder|aria-label)=['"`]([^'"`]*[가-힣\u4e00-\u9fff\u3040-\u309f\u30a0-\u30ff]+[^'"`]*)['"]/g,
  };
  ...
  private async generateReport(
    grouped: GroupedHardcodedStrings
  ): Promise<void> {
    const report = {
      summary: {
        total: Object.values(grouped).flat().length,
        byType: Object.fromEntries(
          Object.entries(grouped).map(([type, strings]) => [
            type,
            strings.length,
          ])
        ),
...
class StringExtractor {
  ...
  private replaceHardcodedString(
    content: string,
    ...
    switch (hardcoded.type) {
      case 'jsxText':
        return content.replace(hardcoded.original, `{t('${translationKey}')}`);
...
class ContextAwareTranslator {
  ...
  async batchTranslate(
    ...
    console.log(
      `🔄 Batch translating ${translationPairs.length} strings to ${targetLocales.length} locales`
    );
    ...
    for (const pair of translationPairs) {
      for (const locale of targetLocales) {
        queue.add(async () => {
          try {
            const result = await this.translateWithContext(pair.value, locale, {
              key: pair.key,
    ...
    // 평균 품질 계산
    results.summary.averageQuality =
      results.completed.reduce((sum, result) => sum + result.quality, 0) /
      results.completed.length;

    console.log(
      `✅ Batch translation completed: ${results.summary.success}/${results.summary.total}`
    );
    return results;
  }
}
...
class QualityChecker {
  ...
  async validateTranslationSet(
    ...
    // 4. 문화적 적합성 검사
    const culturalIssues = await this.checkCulturalAppropriateness(
      translations,
      locale
    );
...
class I18nMonitoringSystem {
  ...
  trackPerformance(operation: string, duration: number, locale: string): void {
    this.metricsCollector.histogram(
      'i18n_operation_duration_seconds',
      duration,
      {
        operation,
        locale,
  ...
  private async queueForAutoTranslation(
    key: string,
    locale: string
  ): Promise<void> {
    // Redis 큐에 추가
...
_Implementation Guide: 실행이 성공을 만든다_
