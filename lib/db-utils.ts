...
export async function updateCountryVisit(
  id: string,
  data: {
    country?: string;
    entryDate?: Date;
...
export async function getUserCountryVisits(
  userId: string,
  limit: number = 100
) {
  const startTime = Date.now();
  ...
  const duration = Date.now() - startTime;
  dbPerformanceMonitor.trackQuery('getUserCountryVisits', duration, {
    userId,
    limit,
  });
...
export async function getSchengenCountryVisits(
  userId: string,
  fromDate: Date,
  toDate: Date = new Date()
) {
  // Use optimized query with caching and compound indexes
  return await OptimizedQueries.getSchengenVisitsOptimized(
    prisma,
    userId,
    fromDate,
    toDate
...
export async function updateNotificationSettings(
  ...
  return await prisma.notificationSettings.upsert({
    ...
    update: {
      ...settings,
      visaExpiryDays: settings.visaExpiryDays
        ? JSON.stringify(settings.visaExpiryDays)
        : undefined,
    },
    create: {
      userId,
...
export async function getUserTravelStats(userId: string) {
  ...
  const countryStats = await OptimizedQueries.getCountryStatistics(
    prisma,
    userId
  );
  ...
  const totalDays = countryStats.reduce(
    (sum: number, stat: any) => sum + Number(stat.total_days || 0),
    0
  );
  const totalVisits = countryStats.reduce(
    (sum: number, stat: any) => sum + Number(stat.visit_count || 0),
    0
  );
  ...
  const schengenStats = countryStats.filter((stat: any) =>
    schengenCountries.includes(stat.country)
  );
  const schengenDays = schengenStats.reduce(
    (sum: number, stat: any) => sum + Number(stat.total_days || 0),
    0
  );
