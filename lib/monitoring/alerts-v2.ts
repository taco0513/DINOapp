...
export class AlertManager {
  ...
  async send(alert: Alert): Promise<void> {
    // Check minimum level
    if (this.config.minimumLevel !== undefined && alert.level < this.config.minimumLevel) {
      return
    }
    ...
    if (this.config.deduplication && alert.deduplicationKey) {
      ...
      if (lastSent && Date.now() - lastSent < (this.config.deduplicationWindow || 300000)) {
        return
      }
    ...
    // Send to channels
    const channelsToUse = alert.channels || [this.config.defaultChannel || 'email']
