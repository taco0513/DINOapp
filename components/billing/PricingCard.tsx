...
export function PricingCard({ plan, currentPlan, onSelectPlan, loading }: PricingCardProps) {
  ...
  return (
    <Card className={`relative p-6 ${isPopular ? 'border-2 border-primary shadow-lg' : ''}`}>
      {/* 인기/할인 배지 */}
      {plan.badge && (
        <Badge 
      ...
      <div className="text-center mb-6">
        ...
        <div className="mb-2">
          ...
          <span className="text-gray-600">/{plan.period === 'month' ? '월' : '년'}</span>
        </div>
    ...
    <div className="py-12">
      <div className="text-center mb-12">
        ...
        <p className="text-gray-600 max-w-2xl mx-auto">
          복잡한 비자 걱정 없이 여행을 즐기세요. 언제든 플랜을 변경하거나 취소할 수 있습니다.
        </p>
      </div>
      ...
      <div className="mt-16 max-w-3xl mx-auto px-4">
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-gray-600">네! 모든 유료 플랜에 7일 무료 평가판이 포함되어 있습니다. 신용카드가 필요하지만 평가판 기간 중 취소하면 요금이 청구되지 않습니다.</p>
          </div>
          <div>
            <h4 className="font-semibold mb-2">언제든 취소할 수 있나요?</h4>
            <p className="text-gray-600">물론입니다. 언제든 계정 설정에서 구독을 취소할 수 있으며, 현재 결제 기간이 끝날 때까지 서비스를 계속 이용할 수 있습니다.</p>
          </div>
          <div>
            <h4 className="font-semibold mb-2">데이터는 안전한가요?</h4>
            <p className="text-gray-600">사용자 데이터 보안이 최우선입니다. 모든 데이터는 암호화되어 저장되며, Google API는 최소 권한 원칙에 따라 필요한 권한만 요청합니다.</p>
          </div>
        </div>
      </div>
    </div>
