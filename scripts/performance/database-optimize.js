...
async function applyIndexes() {
  ...
  const indexesPath = path.join(__dirname, '../../prisma/performance/indexes.sql')
  ...
  const performanceQueries = [
    ...
    },
    {
      ...
      query: `
        ...
        AND "entryDate" >= $2
      `,
      params: ['test-user-id', new Date(Date.now() - 6 * 30 * 24 * 60 * 60 * 1000)]
    }
  ]
  ...
  for (const { name, query, params } of performanceQueries) {
    try {
      ...
      if (explainText.includes('Seq Scan')) {
        console.log(`     ⚠️  Sequential scan detected - consider adding index`)
      }
...
async function generateRecommendations() {
  ...
  try {
    ...
    console.log('     - Implement query result caching for complex calculations')
...
async function healthCheck() {
  ...
  try {
    ...
    try {
      ...
      console.log(`🔗 Connections: ${stats.active_connections} active, ${stats.idle_connections} idle (${stats.total_connections} total)`)
