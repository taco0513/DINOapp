...
export class JSONExporter {
  ...
  public async exportUserData(
    ...
    if (config.categories.preferences) {
      exportData.data.preferences = await this.exportPreferences(
        userId,
        config
      );
    }
    ...
    if (config.categories.notifications) {
      exportData.data.notifications = await this.exportNotifications(
        userId,
        config
      );
    }
...
export class FileParser {
  ...
  private mapCSVRowToTrip(
    headers: string[],
    values: string[]
  ): Partial<CountryVisit> {
  ...
  try {
    ...
    const importConfig: ImportConfiguration = {
      ...
      duplicateHandling: {
        ...
        criteria: config.duplicateHandling?.criteria || [
          'country',
          'entryDate',
        ],
      },
    ...
    if (importConfig.backup.createBackup) {
      backupId = await createUserBackup(
        session.user.id,
        importConfig.backup.backupFormat
      );
    }
