...
test.describe('Critical User Journeys', () => {
  ...
  test('User can access the application and view dashboard', async ({ page }) => {
  ...
  test('User can navigate to Schengen calculator', async ({ page }) => {
    ...
    if (await schengenLink.isVisible()) {
      ...
      await expect(page.locator('form, .calculator, [data-testid*="schengen"]')).toBeVisible()
    } else {
  ...
  test('User can navigate to trips management', async ({ page }) => {
    ...
    if (await tripsLink.isVisible()) {
      ...
      await expect(page.locator('form, .trip, [data-testid*="trip"]')).toBeVisible()
    } else {
  ...
  test('Search functionality works if available', async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      const hasResults = await page.locator('.search-results, .results, [data-testid*="result"]').isVisible()
      const hasNoResults = await page.locator('.no-results, .empty').isVisible()
