  ...
  switch (command) {
    ...
    case 'build-platform':
      ...
      if (platform) {
        ...
        console.error(
          'Please specify a platform: web, ios, android, react-native'
        );
      }
      break;
    default:
      console.log(
        'Usage: node build-tokens.js [build|watch|build-platform <platform>]'
      );
  }
}
...
class DocumentationGenerator {
  ...
  async extractExamples(storyFile) {
    ...
    const exportMatches =
      storySource.match(/export const (\w+): Story = \{([\s\S]*?)\};/g) || [];
