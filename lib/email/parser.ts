...
export class EmailParser {
  ...
  private async extractData(
    ...
    for (const pattern of provider.patterns) {
      ...
      if (pattern.bodyPatterns.confirmation) {
        const confirmation = this.extractFirstMatch(
          content,
          pattern.bodyPatterns.confirmation
        );
        if (confirmation) {
  ...
  private extractFlightData(content: string, pattern: any) {
    ...
    if (pattern.bodyPatterns.flightNumber) {
      result.flightNumber = this.extractFirstMatch(
        content,
        pattern.bodyPatterns.flightNumber
      );
    }

    // 출발지 정보
    if (pattern.bodyPatterns.departure && pattern.bodyPatterns.departureDate) {
      const departureLocation = this.extractFirstMatch(
        content,
        pattern.bodyPatterns.departure
      );
      const departureDate = this.extractFirstMatch(
        content,
        pattern.bodyPatterns.departureDate
      );
    ...
    if (pattern.bodyPatterns.arrival && pattern.bodyPatterns.arrivalDate) {
      const arrivalLocation = this.extractFirstMatch(
        content,
        pattern.bodyPatterns.arrival
      );
      const arrivalDate = this.extractFirstMatch(
        content,
        pattern.bodyPatterns.arrivalDate
      );
  ...
  private extractHotelData(content: string, pattern: any) {
    const checkInStr = this.extractFirstMatch(
      content,
      pattern.bodyPatterns.checkIn || []
    );
    const checkOutStr = this.extractFirstMatch(
      content,
      pattern.bodyPatterns.checkOut || []
    );
    const locationStr = this.extractFirstMatch(
      content,
      pattern.bodyPatterns.location || []
    );
    const nameStr = this.extractFirstMatch(
      content,
      pattern.bodyPatterns.name ||
        pattern.bodyPatterns.hotelName ||
        pattern.bodyPatterns.location ||
        []
    );
  ...
  private calculateConfidence(
    ...
    score +=
      requiredFields.filter(field => data[field as keyof ParsedEmailData])
        .length * 0.2;
    ...
    if (data.type === 'flight') {
      ...
      score +=
        flightFields.filter(field => data[field as keyof ParsedEmailData])
          .length * 0.15;
    }
