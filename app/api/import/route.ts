  ...
  try {
    ...
    for (const tripData of validatedData.trips) {
      try {
        ...
        errorCount++
        errors.push(`Failed to import trip to ${tripData.country}: ${error instanceof Error ? error.message : 'Unknown error'}`)
      }
    }
        ...
        await prisma.notificationSettings.upsert({
          ...
          update: {
            visaExpiryDays: validatedData.notificationSettings.visaExpiryDays 
              ? JSON.stringify(validatedData.notificationSettings.visaExpiryDays)
              : undefined,
            schengenWarningDays: validatedData.notificationSettings.schengenWarningDays,
          ...
          create: {
            ...
            visaExpiryDays: validatedData.notificationSettings.visaExpiryDays 
              ? JSON.stringify(validatedData.notificationSettings.visaExpiryDays)
              : "7,14,30",
            schengenWarningDays: validatedData.notificationSettings.schengenWarningDays || 10,
            emailEnabled: validatedData.notificationSettings.emailEnabled ?? true,
            pushEnabled: validatedData.notificationSettings.pushEnabled ?? false
          }
        })
      } catch (error) {
        errors.push(`Failed to import notification settings: ${error instanceof Error ? error.message : 'Unknown error'}`)
      }
    }
