...
import { createTravelManager } from '../lib/travel-manager.js'
import { getPrismaClient } from '../lib/database/dev-prisma.js'
...
async function testTravelManager() {
  ...
  try {
    ...
    // Test 7: Travel preferences
    console.log('\n⚙️ Test 7: Managing travel preferences...')
    ...
    const preferences = await travelManager.getTravelPreferences()
    console.log('✅ Travel preferences managed:', {
      regions: preferences?.preferredRegions?.length || 0,
    ...
    console.log('- ✅ Travel preferences management working')
    console.log('- ✅ Database schema fully functional')

    // Cleanup
    await prisma.countryVisit.deleteMany({ where: { userId: testUser.id } })
    await prisma.travelPreferences.deleteMany({ where: { userId: testUser.id } })
