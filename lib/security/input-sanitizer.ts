...
export class InputSanitizer {
  ...
  static sanitizeObject<T extends Record<string, any>>(
    obj: T,
    rules: Partial<Record<keyof T, 'text' | 'html' | 'email' | 'url' | 'filename' | 'phone'>>
  ): T {
    const sanitized = { ...obj }
      ...
      if (typeof value === 'string') {
        switch (rule) {
          ...
          case 'url':
            ...
            break
          case 'filename':
            sanitized[key as keyof T] = this.sanitizeFilename(value) as T[keyof T]
            break
          case 'phone':
  ...
  // 배열의 모든 문자열 요소 정화
  static sanitizeArray(arr: string[], rule: 'text' | 'html' = 'text'): string[] {
    return arr.map(item => {
      if (typeof item === 'string') {
        return rule === 'html' ? this.sanitizeHtml(item) : this.sanitizeText(item)
      }
      return item
    })
  }
  ...
  // 부동소수점 값 검증 및 정화
  static sanitizeFloat(input: any, min?: number, max?: number, decimals?: number): number | null {
