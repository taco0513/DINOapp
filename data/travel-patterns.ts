...
export const koreanAirlinePatterns: TravelEmailPattern[] = [
  {
    ...
    extractors: {
      ...
      bookingReference: [/예약\s*번호[:\s]*([A-Z0-9]{6,})/gi, /confirmation[:\s]*([A-Z0-9]{6,})/gi],
      airports: [/([A-Z]{3})\s*→\s*([A-Z]{3})/gi, /\b([A-Z]{3})\s*-\s*([A-Z]{3})\b/gi]
    }
  },
  {
...
export const hotelPatterns: TravelEmailPattern[] = [
  {
    ...
    weight: 0.8,
    extractors: {
      bookingReference: [/booking\s*number[:\s]*([A-Z0-9]{6,})/gi, /예약\s*번호[:\s]*([A-Z0-9]{6,})/gi],
      dates: [
